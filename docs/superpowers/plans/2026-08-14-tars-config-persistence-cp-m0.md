...
디스크 이미지를 굽는 것은 호스트(컨테이너) 쪽 도구다. `mkfs.ext2`는
...
`e2fsprogs`는 게스트에 들어가는 것이 아니라 컨테이너에서 실행되는 도구이므로
...
(1)을 통과했다는 뜻이다. 즉 디스크를 안 물린 체인의 실패 로그가
