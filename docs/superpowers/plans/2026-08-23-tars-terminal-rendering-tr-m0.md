...
공백을 쓰는 이유는 design 결정 7에 있다 — 공백이면 셀 전체가 배경색이라
...
`terminal/src/drm.zig`의 `setPixel` 함수(현재 `:128-132`) 바로 다음 줄에
...
실제로 화면에 나타나는지 확인하는 자리다 — 코드를 안 고치고 넘어가면
...
공백 셀을 쓰는 이유가 이것이다(이번에 정하는 것 2번).
