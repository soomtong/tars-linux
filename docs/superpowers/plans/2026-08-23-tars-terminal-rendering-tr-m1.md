...
| 한글 자모(U+1100~U+11FF) | 64 / 256 |
| **호환 자모(U+3131~U+318E, `ㄱ`·`ㅏ`)** | **0 / 94** |
| **한자(U+4E00~U+9FFF)** | **0 / 20992** |

**완성형 한글은 하나도 빠짐없이 있고, 낱자와 한자는 아예 없다.** `ㄱ`을 찍으면
아무것도 안 나온다는 뜻이다. 우리 입력 경로에 한글 IME가 없어서 지금은 문제가
되지 않지만, 나중에 IME를 붙이면 조합 중인 낱자를 이 폰트로 못 그린다.
