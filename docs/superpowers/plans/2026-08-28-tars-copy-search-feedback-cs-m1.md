...
처리하기 직전 한 자리에서 끈다. `?Prompt`를 만드는 자리가 두 갈래가 된다.
`drawPrompt`는 안 바뀐다.
...
끄는 자리가 `main.zig` 하나뿐이 된다. 그러면 성공한 검색이 앞의 실패를 안
...
### 결정 2. 끄는 자리는 `switch`보다 앞이다

`main.zig`의 `for (keys.copies) |cmd|` 루프에서 `switch` 바로 앞에
`screen.findClearMissed()`를 둔다. 루프 밖이 아니라 안이다.
...
않는다. 그 둘은 이 milestone에서 정확히 갈라야 하는 것이다 — Task 1·2가
`vt.zig`를 고치고 Task 3이 `main.zig`를 고치므로, 창구가 없으면 Task 3의 실수를
게이트가 영영 못 잡는다.

`dumpOverlay(prompt)`가 `render()`에 넘어간 바로 그 값을 받아 찍는다.
...
12초다. `copy` 체인이 게이트에서 세 회차 도므로 전체에 약 36초다.
...
기대: 조용히 끝나고 `vt_test`가 CS-M0까지와 글자 하나 다르지 않은 PASS를
낸다. 아직 새 검사를 안 더했다.
...
`self.hl_spans.clearRetainingCapacity();` 바로 다음 줄이다. 즉
...
다음이다. 즉 `/// 검색 결과. `main.zig`가 로그에 쓴다.` 주석 앞이다.
...
그래도 `zig build`를 함께 도는 이유가 여기 있다 — 이 Task의 실수는
...
`echo "leaving copy mode cleared the highlight"` 바로 다음이다. 즉
...
- `findNeedle()`이 프롬프트가 닫히면 null이라 메시지에 쓸 글자가 `find_last`
  에서 올 수밖에 없다는 것(결정 8과 9가 맞물리는 자리)
- `find_missed`를 "켠다"가 아니라 매번 값을 정한다로 바꾼 이유(plan 결정 1)
- 끄는 자리가 `switch`보다 앞인 것이 두 계약을 한꺼번에 만든다는 것
