...
깨졌음을 확인한다. 그리고 이 ISO를 실기에 꽂는 법을 `README.md`에 적는다.
...
통째로 안 붙는 것이다. `project_target_hardware`가 이 서브프로젝트를 처음
지목할 때 이름으로 삼았던 항목이다.
...
본다"고 적었는데, 그 표는 착수 전에 쓴 것이고 `ACPI_PROCESSOR`는 아예 안
...
16분"이라고 적어 뒀는데 지금은 열 체인 3/3에 20분이다. GL-M3 시점의
문장이고 그 뒤로 체인이 둘 늘었다.
...
- design의 `Status:`를 완료로 고치고 실측 절을 더한다
