...
design의 "막혀 있는 것 넷" 중 **아래 두 층**이다. 위의 둘(`input.zig`의 find
...
마지막 줄이 `PASS`이고 `vt_test`의 마지막 검사가 **51**이다
(`vt_test: copy 커서도 한글 위에서 두 칸이다 OK`). 새 검사는 **52부터**다.
...
안 바뀌는 것이 기대값**이다. 그런데 `findChar`가 껍데기가 됐고
...
시간은 직전 기준선이 **18분 32.80초**다(IS-M1). 이 milestone은 부팅하는
...
| 4 | 경계 검사의 산수가 틀려 아무것도 안 보고 통과한다 | 6 + 120 = **126**, 126 + 3 = 129 > 128이라 거절, 126 + 1 + 1 = **128**이라 수용. 검사가 세 값을 전부 확인한다 |
