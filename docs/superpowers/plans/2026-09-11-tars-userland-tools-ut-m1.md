...
Architecture: 바이너리 목록을 `kernel/guest_tools.sh` 한 파일로 뺀다.
...
— 특히 결정 4 · 결정 7 · 위험 3 · 위험 4와 최종 목록의 "층 1".

---

## 착수 전에 이 세션이 실측한 것 — design 실측 3이 두 곳에서 불완전했다

design 실측 3은 *"새로 필요한 라이브러리는 둘뿐이다 — `sed`가
...
| `tools/check.sh` | 검사 1이 같은 배열을 본다. 화면 검사 셋이 는다 | 고친다 |
...
선다. 그리고 이미지 재빌드는 네트워크를 쓰는 유일한 단계다.
...
`SYSROOT`·`WORKDIR`을 참조하지 않는다 — 데이터만 있는 파일이라
...
`CHAINS` 배열이 서브프로젝트의 실제 상태를 가장 정확하게 말하는 자리다
(CLAUDE.md).
...
같은 종류는 안 온다는 것이 design의 예측이다 — 게이트가 그것을 판정한다.
