...
`/dev/vda`는 virtio-blk에만 있는 이름이다. 노트북에는 virtio가 없다 —
...
넘어간다(`main.zig:79`). 안 되는 것은 설정이 부팅 사이에 남는 것이다.
...
`mountFs`를 고치지 않고 그 앞에 고르는 단계를 넣는다.
...
`/dev/vda`라는 이름이 아니라 디스크 안에 든 표식으로 고른다.
...
여기서는 매체가 없는 광학 드라이브나 카드 리더에서 open이 매달리는 것을
...
12바이트라 여유가 있다. 넘겼을 때 조용히 잘리는 것이 위험이므로 Task 1
검사 5가 그 경계를 본다.
