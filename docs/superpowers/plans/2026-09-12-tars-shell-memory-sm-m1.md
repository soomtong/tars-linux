...
>
> 그리고 **plan에 없던 것을 하나 했다** — 씨앗의 **실제 바이트**를 꺼내
...
| `init/src/config_test.zig` | `expectQuietSeed`를 **정확 허용 목록 양방향**으로 넓힌다(design 결정 6) |
| `config/check.sh` | 부팅 **둘**을 더한다 — 6차가 수리하고 **7차가 훅을 증명한다** |
...
### 1. 씨앗이 한 글자라도 찍으면 **다른 다섯 체인**이 깨진다

설정 디스크를 붙이는 체인이 다섯이고(`config`·`input`·`power`·`hangul`·
`machine`) 그중 셋이 **화면의 셀 좌표**로 판정한다. 씨앗은 그 다섯 전부에서
...
1. 7차의 rc가 **정확히 우리 씨앗**이다 — 게이트가 1차에서 손으로 더한
...
`whence -w`가 보는 것은 그 이름의 **함수**다. 게이트의 판정 글자는
...
alias 둘 **뒤**에 붙인다. 머리 주석의 *"여기 있는 것이 주석과 alias뿐"*도
...
| **역방향** | `hookLines()`의 **전부**가 씨앗에 있어야 한다 | **훅을 지우는 것이 통과하는 것** |
...
| `tars-rc-alive`가 **없다** | 7차의 rc는 사람이 손댄 적 없는 **우리 씨앗**이다 |
...
잡았다** — 내가 만들려던 실패가 아니라 **정방향 검사가 자기 일을 한 것**이다.
고친 뒤 다시 했다.
