...
Task 1·2가 앞인 이유는 PM-M0·PM-M1과 같다. 부팅 20초를 쓰기 전에 0.1초로
...
Task 3이 Task 4보다 앞인 이유는 로그 문구 때문이다. 게이트가 grep할
...
`EV_KEY`는 1번이다. 0으로 읽으면 `EV_SYN`만 가진 장치가 전부 키보드로
...
에서 `}` 앞에 아래 블록을 넣는다(마지막 `}`는 그대로 두고 그 위에 끼운다).
...
바로 다음에 아래 블록을 넣는다(`pub fn main` 앞이다).
