...
것은 다른 파일이고, 지금까지 그 둘의 차이를 아무도 보지 않았다. ACPI처럼
...
`drivers/acpi/Kconfig`에서 `default y`인 항목 중 프롬프트가 있는 것만
...
없어야 한다는 음성 검사 한 줄로 둘을 가른다. 같은 구별을 훨씬 싸게 얻고,
...
게이트 한 번에서 커널 빌드가 차지하는 몫이 15 × 50.4초 ≒ 12분 36초이고,
...
3. `input: Power Button as /devices/...` 와 `input: AT Translated Set 2
   keyboard as /devices/...` — 두 장치의 등록 순서가 여기서 보인다.
...
같은 파일에서 부팅 1의 마지막, `echo "boot 1/2 PASS: ..."` 바로 위에 있는
...
지우는 것은 부팅 1의 것 하나뿐이고, Step 6의 `rg`가 그것을 개수로
확인한다.
