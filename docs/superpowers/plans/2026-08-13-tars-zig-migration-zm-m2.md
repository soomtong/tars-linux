...
순서는 소스 → 이미지다. 소스 삭제 후 옛 이미지(Rust가 아직 있는)로 TF
...
Expected — 나오는 줄은 지울 파일 자신과 지울 주석뿐이어야 한다:
...
첫째, `clean()` 바로 위 15~16번째 줄을 삭제한다. kms가 없어졌으므로
...
동작하므로 건드리지 않았다. 깨진 게이트와 손으로 쓰는 도구를 구분한 것이
...
유저랜드가 그대로라는 실질 증거다. `FROM`과 apt `RUN` 레이어는 명령도
