...
가는 것 전용 최적화 옵션을 만들어 기본값을 `ReleaseSafe`로 둔다. 호스트 검사
...
clean 빌드 71.0초, 둘 다 박으면 10,577,200바이트에 70.9초다.
...
섹션이 남아 있는 것뿐이고, 그것이 곧 "strip과 다르다"의 증명이다.
...
1. 여덟 체인 3/3 통과, 세 번 모두. 최적화 모드를 바꾸는 것은 검증 대상
   바이너리 자체를 바꾸는 일이고 TF-M4가 이 변경을 미룬 이유가 정확히
   이것이었다. 여덟 체인 전부가 그 바이너리를 딛고 서 있으므로 통과가 곧
   증명이다.
2. 시간. 기준선 19분 11초~16초에서 크게 안 변하는 것이 예측이다.
...
> Task 4 결과 (2026-08-29). 네 가지를 다 보았다.
>
> | | 1회 | 2회 | 3회 | 폭 |
> |---|---|---|---|---|
> | GL-M2 뒤 | 19:16 | 19:11 | 19:12 | 5초 |
> | GL-M3 뒤 | 16:11 | 16:10 | 16:01 | 10초 |
>
> 세 번 다 `TARS check PASS`이고 `skipping make`도 세 번 다 23회다.
> `terminal` 10,577,208바이트 · initrd 10,988,773바이트,
> `.debug_*` 섹션 열 개가 그대로 있다.
>
> 게스트 실행 시간이 이 milestone의 본체다.
>
> | | Debug | ReleaseSafe |
> |---|---|---|
...
>
> 위 Step 1의 2번 예측("크게 안 변한다")이 틀렸다. 3분 02초가 더 줄었다.
> copy 체인 단독이 143·142·139초 → 129·129·129초이고, 그 13초가
> GL-M2 뒤 타이핑의 폴링 부분(151키 × 0.085초 = 12.8초)과 거의 정확히 같다.
>
> GL-M2가 게이트의 대기를 "고정 시간"에서 "게스트 응답 시간의 측정"으로
> 바꿔 놓았기 때문이다. GL-M3이 게스트를 열 배 빠르게 만들자 그것이 게이트
> 시간으로 흘러들어 왔다. GL-M3을 먼저 했다면 정말로 본전이었을 것이다 —
> 고정 0.3초 대기는 게스트가 아무리 빨라져도 그대로다. 순서가 값을 했고
> 그것은 계획한 것이 아니었다.
>
> `key_bytes`는 세 회차 모두 95로 GL-M2와 같았다 — 빨라졌는데도 키를
> 하나도 안 놓쳤다.
...
- 빌드가 안 되면 fortify가 남은 자리가 더 있는 것이다. 착수 전 측정은
  `input.zig`와 `font.zig`가 무관하다고 보았지만, 그것은 지금 소스 기준이다.
