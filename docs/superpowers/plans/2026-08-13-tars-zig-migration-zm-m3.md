...
컴파일 또는 amd64 패키지 조달로 만든다. 에뮬레이션 층이 두 겹(Rosetta로
...
만드느냐"다. 산출물의 타깃은 전부 이미 x86_64로 명시돼 있어서
...
순서는 조사 → 이미지 → 커널 → Zig → initrd → 체인 1회씩 → 전체 게이트다.
design doc이 이 milestone을 마지막에 둔 이유가 "실패 모드가 넓다"인데, 그
대책은 하나 바꾸고 그 자리에서 확인하는 것뿐이다. 그래서 게이트 전체를
...
헤더를 읽으므로 Zig 번들에 그게 없으면 막힌다고 적었다. 실제 파일을 보면
그렇지 않다.
...
셋 다 glibc 헤더이고, DRM 쪽은 구조체를 `extern struct`로 손으로 선언해
...
빼는 것이 매번 중요하다. 붙인 채로 arm64 이미지를 돌리면 Docker가 다시
...
이 여덟 줄은 Task 1의 실측에서 나온 최소 집합이다. 옛 initrd에 실제로
...
1. `arch/x86/boot` 또는 `arch/x86/realmode`에서 `-m32`/`-m16` 관련 에러 →
   ...
   이건 아닐 가능성이 크다.
2. `tools/objtool` 또는 `arch/x86/tools/relocs` → 이건 호스트 도구라
...
- design doc — Status를 `ZM-M3 complete`로. ZM-M3 절에 ① 리스크로 적었던
  `drm.zig` `@cImport` 문제가 처음부터 오해였다는 정정(실제로는 glibc
