...
M2가 처방을 코드에 넣었다(실측 17). `net.zig`의 argv에 `-o ntp_servers`가
있고, 그것을 주고도 dhcpcd가 게이트에서 그대로 산다. 옵션 이름을 dhcpcd가
파일 없이 아는 것도 sysroot에서 확인했다 — `/usr/share/dhcpcd`에 정의 파일이
없는데 바이너리 안에 `define 42 array ipaddress ntp_servers`가 박혀 있다.

남은 미지수가 하나로 줄었다. 실기계의 DHCP 서버가 실제로 option 42를 주는가
— 게이트는 그것을 영영 못 본다(결정 4의 3번). 안 주면 증상이 부팅 로그의
`tars-init: gave up waiting for /run/tars/ntp_servers` 한 줄이고, 그것이
실측 20이 만든 진단의 시작점이다.

아래는 처방이 정해지기 전의 기록이다.
...
둘 다 실측으로 나왔고 예상이 컸다. 부팅 A가 +15.334초이고(33.889 →
49.223초, 실측 15) 부팅 B가 +3.1초다(50.581 → 53.7초, 실측 21). 합이 약
18.4초이고 게이트가 체인을 세 번 도니 약 55초다 — 예상 75초보다 작다.

부팅 A와 B의 크기가 다섯 배 갈린 이유는 기다림이다. A는 자식이 리스를
기다리며 약 10초를 쓰는 것을 체인이 지켜보고(실측 10), B는 파일이 이미
있어서 아무도 안 기다린다(실측 18).
...
plan은 `docs/superpowers/plans/2026-09-15-tars-time-sync-ts-m2.md`이고 거기서
design에 없던 결정 여섯(M2-A~M2-F)을 먼저 정했다. 부팅 B가 첫 회에 초록이어서
TS 자신이 고칠 것은 하나도 없었는데, 대신 이 체인이 오래 갖고 있던 거짓
판정 둘이 드러났다(실측 22 · 23). 실측이 여덟이고 마지막이 게이트다.

### 실측 16 — 이어 붙인 cpio 조각을 커널이 푼다

부팅 B의 initrd는 `kernel/initrd.cpio`(40,565,420바이트, gzip) 뒤에 175바이트
짜리 gzip cpio 한 덩이를 `cat`으로 이어 붙인 것이다. 그 덩이에 든 것은
`run/tars/ntp_servers` 파일 하나이고, 게스트의 init이 그것을 읽었다.

즉 결정 M2-D가 기댄 성질이 실제로 성립한다 — initramfs 언패커가 버퍼를 다 쓸
때까지 archive를 이어 읽고 뒤의 것이 앞의 것을 덮는다. 공용 산출물인
`kernel/initrd.cpio`를 한 바이트도 안 건드리고 부팅 하나만 다른 트리로 띄울 수
있다는 뜻이라, 앞으로 "이 부팅에만 파일 하나"가 필요한 체인이 쓸 수 있는
수법이다.

### 실측 17 — `-o ntp_servers`를 준 dhcpcd가 그대로 산다

design 위험 3의 처방이 게이트에서 아무것도 안 깼다. 검사 4~10(주소 ·
`/etc/resolv.conf` · 기본 경로 · TCP 연결 · dhcpcd 생존)이 전부 초록이다.

착수 전에 sysroot에서 확인한 것이 이 결과를 설명한다 —
`/usr/share/dhcpcd`에 정의 파일이 없는데 바이너리 안에
`define 42 array ipaddress ntp_servers`가 박혀 있다. 즉 dhcpcd가 옵션 이름을
파일 없이 안다. 그 반대였다면 증상이 "주소가 안 붙는다"로 나와서 우리가 고친
자리와 멀었을 것이다.

SLIRP는 여전히 option 42를 안 준다. 게스트에 `/run/tars/ntp_servers`가 생긴
회차가 하나도 없고, 그래서 부팅 B가 심은 파일을 dhcpcd가 덮어쓰는 일도 안
일어난다(검사 21이 주소까지 함께 보므로 그 일이 생기면 알린다).

### 실측 18 — 안 닿는 서버가 부팅에 0초를 더한다

design 결정 3의 음성이 수로 나왔다. 부팅 A가 3초, 부팅 B가 3초이고 다섯 판을
연속으로 돌려도 둘 다 3초였다 — 차이가 0초다.

부팅 B의 자식은 이때 `192.0.2.1`에 답 없는 질문을 하는 중이고, 그 일이
부모를 한 순간도 안 세운다는 것이 이 0초다. 막혔다면 붙는 시간이 60초
이상이다(자식이 30회 × 2초를 쓴다).

부수로 알게 된 것 하나. 이 게스트가 프롬프트까지 3초다 — plan이 12초 안팎을
예상했는데 네 배 빠르다. 그래서 검사 22의 상한 10초는 잡음보다 훨씬 위에
있다.

### 실측 19 — 자식이 살아 있는 채로 전원을 받아도 유예를 안 쓴다

TS-M1 결정 M1-A의 진짜 시험이 이 부팅이었고 통과했다. 부팅 B는 프롬프트를 본
뒤 몇 초 만에 `system_powerdown`을 받는데, 그때 SNTP 자식은 서른 번 중 몇
번째를 묻는 중이라 분명히 살아 있다. 그런데도 `grace period expired`가 한
줄도 안 나왔다 — 자식이 SIGTERM에 즉시 죽었다는 뜻이고, 그것이
`power.resetToDefault()`가 하는 일 전부다.

M1에서는 이 사실을 증명할 수 없었다. 그 부팅의 자식은 시계를 뛰고 스스로
끝나므로 함수가 없어도 초록이 된다.

### 실측 20 — 반사실이 겨냥한 자리에서 죽었다

심는 줄 하나만 뺀 사본은 검사 20을 초록으로 지나고 검사 21에서 죽는다.
게스트 로그에 셋이 순서대로 남는다.

```
tars-init: ntp=dhcp, waiting for /run/tars/ntp_servers
tars-init: sntp child (pid 35) will ask dhcp
tars-init: gave up waiting for /run/tars/ntp_servers
```

그 셋이 "자식이 태어나서 기다리다 포기했다"와 "자식이 아예 안 태어났다"를
가른다. 기다린다고 먼저 알리는 줄(결정 M2-A)이 없으면 이 진단이 30초 침묵
하나가 된다.

순서에도 값이 있다. 자식의 첫 줄이 부모의 줄보다 먼저 찍혔다 — `fork` 뒤에
자식이 먼저 스케줄됐다는 뜻이고, 로그를 읽는 사람이 "부모가 자식을 기다렸나"로
오해하지 않도록 적어 둔다.

### 실측 21 — 체인 단독이 49.223초에서 56.046초가 됐다

네 번을 잰 값이 이렇게 쌓인다.

| 시점 | 체인 단독 |
|---|---|
| M2 착수 시점(M1의 49.223초와 잡음 안) | 50.581초 |
| hook 호스트 검사만 더한 뒤 | 51.668초 |
| 부팅 B와 검사 셋까지 더한 뒤 | 53.6~53.7초 |
| 흔들리는 검사 둘을 고친 뒤(리스너 타이핑 12키가 늘었다) | 56.046초 |

M2 전체의 증가분이 5.465초이고 게이트가 체인을 세 번 도니 약 16초다.

부팅 하나에 3초뿐인 이유가 실측 18이다. 위험 5가 예상한 "부팅 둘이면 약
75초"는 절반이 M1의 15.3초였고 나머지 절반이 이것이다 — 예상이 컸다.

### 실측 22 — 이 체인의 드라이버 검사가 타임스탬프에 걸렸다 (TS 밖의 고장)

검사 11이 켜지 않은 NIC 드라이버 이름을 시리얼 로그에서 찾는데, 그 목록에
`8139`가 있었다. 순수 숫자라 커널 printk의 타임스탬프에 걸린다.

```
[    1.381391] clocksource: tsc: mask: 0xffffffffffffffff ...
```

부팅 시각이 그 값일 때만 빨간불이 되므로 회차마다 흔들린다 — 2026-09-15에
같은 체인을 열일곱 판 돌려 한 판에서 봤다. TS가 만든 고장이 아니라 NW-M2
시절부터 있던 것이고, 이 사이클이 체인을 스물네 판 돌리면서 드러났다.

고친 방법은 커널이 실제로 찍는 모듈 이름 둘(`8139cp` · `8139too`)로 바꾼
것이다. `\b`를 붙이는 길은 안 골랐다 — 소수점 바로 뒤(`[    2.813900]`)에서는
그 경계가 서기 때문에 함정이 좁아질 뿐 안 없어진다.

### 실측 23 — 배경 리스너가 터미널을 읽으려다 멈춘다 (TS 밖의 고장)

검사 15가 열일곱 판 중 두 판에서 죽었고 루트 게이트 한 판도 그 자리에서
죽였다. 화면을 복원하니 원인이 한 글자로 드러났다.
...
job을 멈춘다. 멈춘 `nc`는 받은 바이트를 파일에 안 쓰므로 검사 15가 빈 파일을
본다 — "길이 안 섰다"와 증상이 같다.

8080의 리스너가 이 병에 안 걸린 이유도 같은 사실이 설명한다. 그쪽은
`echo … | nc -l -p 8080 &`이라 stdin이 파이프이고 터미널이 아니다.

처방은 `< /dev/null` 한 조각이다. 고친 뒤 일곱 판 연속으로 초록이고, 화면의
같은 자리가 `has ended`로 바뀌며 `cat`이 `inm2-reverse-ok`를 찍는다. 이
저장소에서 `<`(shift-comma)를 처음 쓴 자리이기도 하다.

본문은 `docs/decisions/project_gate_screen_echo.md`에 절로 옮겨 두었다.

### 실측 24 — 루트 게이트가 열두 체인 3/3으로 32분 54.90초다

TS가 처음 돌린 게이트다. 첫 판은 `net` 체인에서 죽었는데 우리가 더한 것이
아니라 실측 23의 흔들림이었고, 그것과 실측 22를 고친 뒤의 판이 초록이다.

기준선이 IN이 닫힐 때의 31분 51.46초이고 차이가 +1분 03.44초다. 설명되는
값이다 — M1이 체인에 15.334초를, M2가 5.465초를 더했고 게이트가 그것을 세 번
도니 약 62초다. 게이트 잡음이 ±3분이라 이 크기는 어차피 안 갈린다.

`skipping make`는 35회로 `12 × 3 − 1` 그대로다. 체인 수가 안 늘었다는 뜻이고,
TS는 `net` 체인에 부팅 둘을 얹었을 뿐이다.
...
### TS-M2 — DHCP가 알려 준 서버를 쓴다 (끝났다)

⚠ plan이 결정 여섯을 더 정했다 — 기다림을 자식 안에 두느라 `sync()`가 fork를
먼저 하는 것(M2-A) · hook이 저장소 파일인 것(M2-B) · hook이 파일 경로를
변수로 한 번 정하는 것(M2-C) · 부팅 B의 initrd를 cpio 조각으로 만드는 것
(M2-D) · 죽은 주소가 `192.0.2.1`인 것(M2-E) · "제때 떴다"를 부팅 A와의
차이로 재는 것(M2-F).
