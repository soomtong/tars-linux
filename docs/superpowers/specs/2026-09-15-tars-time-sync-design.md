...
렌더 하나가 M0에서 안 닫혔는데(실측 7) TS-M1의 부팅 A가 닫았다(실측 12).
시계가 5년 뛴 뒤에 화면 셸에 25키를 치고 그 출력을 읽는 판정이 섰다 —
`terminal`의 렌더 루프도 `type_keys`가 보는 로그 성장도 점프를 지나서
정상이다. 위험 2가 통째로 닫혔다.
...
절반이 실측으로 나왔다(실측 15). 부팅 A가 +15.334초이고(33.889 → 49.223초)
게이트가 체인을 세 번 도니 약 46초다. 예상 12~13초보다 조금 큰데 그 차이가
실측 10의 리스 대기다 — 자식이 주소를 기다리며 약 10초를 쓴다. 남은 절반은
M2의 부팅 B가 잰다.
...
일곱이다. 부팅 A가 첫 회에 초록이어서 고칠 것이 하나도 없었고, 실측 10과
11은 plan이 예상 못 한 것이다. 부팅 셋(체인 두 판과 반사실 한 판)으로
끝났다.

### 실측 9 — 우리 코드가 판 SNTP 왕복으로 시계가 뛴다

```
tars-init: sntp child (pid 35) will ask 10.0.2.2
tars-init: clock stepped to 1930367167 (was 1789431121)
```

`1789431121`이 2026-09-15이고 `1930367167`이 stub이 정한
2031-03-04T05:06:07Z다. 그리고 화면에서

```
tsyear=2031
```

즉 `clock_settime`이 성공을 돌려준 것과 게스트의 `date`가 그 값을 찍는
것이 둘 다 성립한다. 검사 17·18·19가 그 셋을 나눠 본다.

stub 쪽 로그는 왕복 한 번이 전부다.

```
sntp-stub: recv 48 bytes from 127.0.0.1:36259
sntp-stub: sent 48 bytes (origin echoed)
```

재전송이 없다 — 첫 답이 검증 넷을 다 통과했다는 뜻이다.

### 실측 10 — 자식은 주소를 기다린다. 재시도 상한 10은 부족했을 것이다

`sync()`가 불리는 자리는 `net.bringUp()` 바로 다음이라 그 시점에 dhcpcd가
아직 주소를 못 받았다. 그래서 자식의 첫 `sendto`들이 전부 실패한다.

```
tars-init: sntp send failed (errno 101), try 1
…
tars-init: sntp send failed (errno 101), try 10
tars-init: clock stepped to 1930367167 (was 1789431121)
```

errno 101이 ENETUNREACH다. 열 번 실패하고 열한 번째에 성공했다 — 한 회가
`sendto` + 1초 쉼이므로 리스가 붙기까지 약 10초를 재시도로 덮은 것이다.

이 숫자가 M1 plan의 결정 M1-B를 실증한다. 상한을 10으로 잡았다면 정확히
한 회 모자라서 빨간불이었다. 30으로 잡은 근거(NW-M2 실측 17의 리스 지연)가
맞았고, 이 값을 줄이려는 사람은 이 실측을 먼저 본다.

반사실 회차에서는 같은 자리가 여덟 번이었다. 리스가 붙는 시각이 회차마다
흔들린다는 뜻이고, 그래서 상한에 여유가 필요하다.

### 실측 11 — 같은 datagram의 양 끝이 서로 다른 주소를 본다

게스트 쪽 로그에 `sntp answer came from …` 줄이 하나도 없다. 그 줄은 응답의
소스 주소가 우리가 물은 주소와 다를 때만 나오므로, 게스트가 본 소스는
`10.0.2.2`다.

그런데 컨테이너 쪽에서 본 소스는 `127.0.0.1`이다(위 stub 로그와 M0 실측 2).
같은 왕복인데 양 끝이 서로 다른 주소를 본다 — SLIRP가 가운데서 양쪽을
NAT하기 때문이다.

그래서 소스 주소를 거부 조건으로 승격할 수 있다는 것이 확인됐다. 지금은
로그만 찍는다(design 결정 12의 판단은 nonce가 한다) — 승격은 실기에서 같은
성질이 성립하는지 본 뒤의 일이다.

### 실측 12 — 렌더가 시계 점프를 견딘다. 실측 7의 숙제가 닫혔다

M0의 하네스로는 "렌더가 살아 있다"와 "화면에 할 일이 없다"가 같은 값이라
못 갈랐다. 부팅 A는 시계가 뛴 뒤에 화면 셸에 25키를 치고 그 출력을 읽으므로,
검사 19가 선 것 자체가 갈림의 답이다.

```
=== typing 'echo tsyear=$(date -u +%Y)' ===
the guest shows 2031 — the render survived the jump too
```

`terminal`의 렌더 루프도 `type_keys`가 보는 로그 성장도 전부 5년 점프를
지나서 정상이었다. 위험 2의 마지막 조각이 닫혔다.

### 실측 13 — `SO_RCVTIMEO`가 실제로 걸려 있다

stub을 안 띄운 반사실이 그것을 보였다.

```
tars-init: sntp send failed (errno 101)   × 8
tars-init: sntp got no answer (errno 11)  × 22
tars-init: sntp gave up after 30 tries
```

errno 11이 EAGAIN이고, 그 값이 나오는 유일한 길이 수신 타임아웃이다.
`setsockopt`이 조용히 실패했다면 자식이 첫 회에서 영영 매달려 이 줄이 하나도
안 나온다. 즉 이 스물둘은 design 결정 3의 안쪽 겹이 서 있다는 직접 증거다.

여덟과 스물둘을 더하면 정확히 30이다 — 상한이 지켜진다.

### 실측 14 — 반사실이 겨냥한 자리에서 죽었다

stub 자리에 `sleep 600`을 넣은 사본을 마운트해서 돌렸다. 결과가 이렇다.

| 검사 | 결과 |
|---|---|
| 1~16 | 초록(앞의 부팅은 stub과 무관하다) |
| 17 | 초록. 로그에 `net=dhcp ntp=10.0.2.2` 줄이 있다 |
| 18 | 여기서 죽었다. `clock stepped`가 0회 |
| 19 | 도달 못 함. `tsyear` 타이핑이 0회 |

앞의 검사에 걸려 죽지 않은 것이 중요하다. SL-M2의 `.HUP` 반사실은 겨냥한
음성이 아니라 앞의 양성에서 죽어서 아무것도 증명하지 못했는데, 이번에는
검사 17이 초록으로 지나고 18에서만 갈렸다 — 설정 경로와 시계 경로가 서로
안 엮여 있다는 뜻이라 검사를 둘로 쪼갠 값이 있다.

### 실측 15 — 체인 단독이 33.889초에서 49.223초가 됐다

위험 5가 예상한 것이 부팅 하나에 12~13초였고 실제가 +15.334초다. 게이트가
체인을 세 번 도니 약 46초가 붙는다.

잡음(±3분)보다 훨씬 작아서 게이트 전체에서는 안 갈린다. 부팅 A의 15초 중
약 10초가 실측 10의 리스 대기라는 것이 이 숫자의 대부분이다.
...
### TS-M1 — 우리 코드가 시계를 뛴다 (끝났다)

⚠ 순수 함수 넷 중 `parseIpv4`는 `config.zig`에 놓였다. `sntp.zig`에 두면
`config.zig`와 import가 순환하기 때문이고, 근거는 M1 plan의 결정 M1-C에
있다. 그리고 plan이 결정 셋을 더 정했다 — 자식이 시그널 정책을 되돌리는
것(M1-A) · 자식이 주소를 기다리며 재시도하는 것(M1-B, 실측 10이 그 값을
증명했다) · `ntp=dhcp`도 M1에서 파일을 한 번 읽는 것(M1-D).
