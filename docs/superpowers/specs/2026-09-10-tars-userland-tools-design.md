...
Date: 2026-09-10
Status: 완료 — UT-M0·M1·M2·M3(2026-09-10 · 2026-09-11 ×3).
milestone 넷이 전부 끝났다. M0이 `PATH`와 뼈대 넷을 세우고 열한번째 체인을
등록했고, M1이 목록을 한 파일로 빼고 그 위에 GNU 한 벌(도구 50)을 세웠고,
M2가 모던 열셋을 더해 63이 됐고, M3이 git과 `vim.tiny`를 더해 65와 링크
넷이 됐다. 착수 전 기준선은 열 체인 3/3으로 20분 29.84초(RM-M3
시점)였고 M0 뒤가 21분 09.60초, M1 뒤가 21분 35.63초, M2 뒤가
23분 02.73초였다 — M3 뒤의 값은 아래 실측 49에 있다.

게스트에서 `git init` → `add` → `commit` → `log`가 한 번에 돌고, 전역
설정이 `/config`에 남는다.
...
실제로 써 보고 발견한 것이고, 그 점이 이 후보의 무게다.

앞의 서브프로젝트들은 전부 기계가 켜지는 쪽을 만들었다 — 부팅하고, 그리고,
...
| `sleep` | IP-M0이 Ctrl+C로 죽일 자식이 필요해서 |
...
게스트의 모든 프로세스의 환경변수는 커널이 PID 1에게 준 두 개가 전부다.
...
PID 1인지, 터미널인지, 셸 설정인지는 설정 시스템과 함께 결정할 문제이고,
...
새로 필요한 라이브러리는 둘뿐이다 — `sed`가 `libacl1`(74KB),
...
바이너리 크기가 아니라 딸려 오는 라이브러리다. 바이너리별로 쟀다.
...
보이지만 168개 중 대부분은 `git` 자신에 대한 하드링크이고, 별도 실체인
것은 네트워크 헬퍼 일곱이다.

> ⚠️ "하드링크"가 아니라 심볼릭 링크다 — 아래 실측 39. 168 = 심볼릭
> 링크 141 + 실체 26이고, 실체 26 중 큰 것 일곱이 그 네트워크 헬퍼다.
> 결론(안 넣는다)은 안 바뀌고, UT-M3은 한 걸음 더 갔다 — 그 트리를
> 통째로 안 넣는다.
...
`helix`는 trixie에 없어 결정 3(조달 경로 하나)을 깨는 유일한 항목이 되고,
`neovim`은 라이브러리 9개 + 런타임 24MB로 UT 전체보다 무거운 축이다.
사용자가 "가벼운 것 하나를 먼저"로 정했다. 편집기를 제대로 올리는 것은 런타임을
어디서 자를지가 본체인 별도의 일이다.
...
`/usr/bin`을 본다. 그래서 `terminal`의 `setenv` 옆이 아니라 PID 1이다.
...
`/bin/sh`가 `bash`인 이유는 셋 중 유일하게 POSIX sh 모드를 갖고 있어서다
...
`check.sh`의 initrd 목록 검사가 같은 배열을 볼 수 있다.
...
배열이 서브프로젝트의 실제 상태를 가장 정확하게 말하는 자리다.
...
| UT-M2 ✅ | 층 2(모던 13개 — 열둘 + `btop`) | `libgit2` 사슬이 실제로 딸려 오는가 · 이름이 `fd`/`bat`인가 |
...
화면 셸에 보낸다. 시리얼로 치려면 chardev 소켓 배관을 새로 놓아야 하고,
그것은 M0의 일이 아니다.
...
층 2 — 모던 (~~27.3 MB~~ → 실제 45.6 MB, 새 라이브러리 `libgit2` 사슬
~~15~~ 16 · `libncursesw6` · `libjq` · `libonig` · `libstdc++6`)

```
eza  bat  fd  rg  sd  procs  htop  btop  tree  duf  ncdu  jq  hyperfine
```

`btop`은 사용자가 2026-09-11에 더한 것이다 — 이 목록을 쓸 때는 열둘이었다.
그리고 27.3 MB는 `.deb` 압축 크기가 섞인 틀린 값이다(실측 32). 사슬이
15가 아니라 16인 것은 실측 30에 있다.
...
> M3 뒤의 실제 값. 바이너리 둘이 5,844,472바이트이고 새 라이브러리는
> 정말로 0이었다(실측 38 — 이 문장이 M1·M2에서 두 번 틀린 뒤 처음
> 맞았다). 그리고 이 목록에 적히지 않은 것이 더 있었다: `git-core`가
> 아니라 alternatives 이름 둘(`pager` · `editor`)이고, 없으면
> `git log`와 `-m` 없는 `git commit`이 죽는다(실측 42). 링크로 건다.
...
> M2 뒤의 실제 값. initrd는 층 3을 넣기 전에 이미 84.4 MB(풀었을 때,
> gzip 32.0 MB)다. 라이브러리도 20이 아니라 M1이 넷 · M2가 열아홉 = 스물셋
> 이었다 — 이 문단이 두 번(실측 22 · 30) 틀렸고 둘 다 `.so`의 `DT_NEEDED`를
> 안 본 것이 원인이다. 크기 예측은 거의 맞았고 라이브러리 개수는 계속
> 모자랐다는 것이 이 항목의 요지다.
...
TF-M2 시절 53MB에서 부팅조차 못 한 벽이 있었다. limine이 BIOS INT13h로
ISO를 읽는 경로가 에뮬레이션에서 극단적으로 느리기 때문이다
(`kernel/make_initrd.sh`의 주석에 남아 있다).

지금 limine이 읽는 것은 gzip 11.1MB이고, 이 계획 뒤에는 약 30MB로
...
아니라 언제나 빨강이었을 것이다 — 다행히 그 종류의 실패는 첫 실행에서
...
것은 CN-M1이 같은 함정을 의도적으로 피한 것이고, 그 주석이 "프롬프트
...
아무것도 덮어쓰지 않았으므로 명령의 출력은 하나도 안 달라졌다. 달라진
것은 프롬프트이고, 그것은 도구가 아니라 뼈대(결정 6)가 만든 변화다.
...
아니었다"의 거울상이고, 음성 확인이라는 수법 자체가 이 함정을 부른다
(코드를 일부러 뒤로 되돌리는 것이 그 수법의 본체이기 때문이다).

### 실측 19. 실패의 모양이 예측한 그대로였다

`environ_test.zig`를 먼저 쓰고 돌리니 컴파일 에러였다.
...
말로 알려 주므로, 검사가 "무엇이 없다"를 말하고 로그가 "그래서 무엇이
일어났는가"를 말한다.
...
착수 전 기준선 20분 29.84초(RM-M3 시점, 열 체인)에서 +39.76초다.
...
그 앞에 부팅 없이 initrd 목록을 훑는 정적 검사를 둔다(IP 체인과 같은
...
실측 3은 *"새로 필요한 라이브러리는 둘뿐이다 — `sed`가 `libacl1`,
...
문서가 틀렸던 것이고, Dockerfile의 `apt-get download` 목록은 사람이
...
### 실측 24. 정적 검사는 tautology다 — 음성 확인 둘이 갈렸다

결정 7이 *"`check.sh`의 initrd 목록 검사가 같은 배열을 볼 수 있다"*고
적었다. 그렇게 만들고 나서 그 검사가 무엇을 못 보는지를 실행으로 물었다.
...
망가뜨릴 자리도 하나가 되는 것이고, 그 자리가 게이트 첫 판정에서 즉시
드러나는 것이 이 구조가 안전한 이유다.

그래서 `ps`만 겨냥한 두 번째 음성 확인을 따로 했다 — `case`로 `ps`와
...
치면 체인이 그 자리에서 매달리고, 증상은 실패가 아니라 타임아웃이라
...
떠 있는 것을 화면에서 본 것이 이 항목의 값이다(환경변수가 같다는 것까지
...
UT-M0의 21분 09.60초에서 +26.03초다(RM-M3 시점 열 체인
...
2026-09-11 세 번째 세션. 목록이 50에서 63이 됐고, `make_initrd.sh`는
한 글자도 안 고쳤다 — M1의 결정 7이 값을 낸 자리다. 그런데 계획에 없던
것 둘이 나왔고 둘 다 게이트를 통째로 깼다.

### 실측 30. 사슬이 15가 아니라 16이다 — 그런데 Dockerfile은 안 고쳤다

착수 전에 컨테이너 안에서 `.deb` 서른둘을 풀고 `copy_lib_deps`와 같은
규칙으로 closure를 다시 쟀다.

```
libgit2.so.1.9  →  libgssapi_krb5.so.2  →  libkrb5.so.3  →  libresolv.so.2   ← 없던 줄
                                                         →  libcom_err.so.2
                                                         →  libkeyutils.so.1
                →  libssh2.so.1         →  libcrypto.so.3   ← libgit2 직접이 아니다
```

실측 5의 표는 `libcrypto`를 `libgit2`가 직접 끄는 것처럼 적었지만 실제로는
`libssh2`를 거친다. 그리고 `libresolv.so.2`가 그 표에 아예 없다 —
Kerberos가 이름을 풀려고 데려온다.

그런데 Dockerfile에는 안 적었다. `libresolv`는 `libc6`이 담고 있고
그것은 이미 sysroot에 있다 — 적었으면 apt가 "없는 패키지"라고 죽었을 것이다.
`MISSING`이 0이라는 것을 빌드 전에 알았고, 실제로 빌드가 한 번에 통과했다.
M1이 세 번 틀렸던 자리(실측 22)를 30초로 막았다.

### 실측 31. `btop`은 4.01MB이고 `libstdc++`의 유일한 사용자다

| | |
|---|---|
| 바이너리 | 1,510,496 |
| 새 라이브러리 | `libstdc++.so.6` 2,497,768 하나 |
| 테마 | 63,503바이트. 안 넣었다 — 내장 Default로 돈다 |

기존 50개와 층 2의 나머지 열둘 전부의 `DT_NEEDED`를 확인했다. btop을 빼는
사람은 `libstdc++6`도 함께 뺀다 — 그 사실이 `guest_tools.sh`와 Dockerfile
양쪽 주석에 있다.

### 실측 32. 크기 — design의 27.3MB는 `.deb` 압축 크기가 섞인 값이었다

```
initrd gzip   13,434,703 → 32,007,022 bytes   (+18,572,319, 2.4배)
initrd plain  38,840,832 → 84,417,024 bytes
usr/bin 항목            50 → 63
새 라이브러리                    열아홉
make_initrd.sh 전체                 3.449초
```

`gzip -6`은 그대로 둔다. 압축이 비용의 자리가 아니다 — initrd를 2.4배로
키우고도 굽는 데 3.4초다. UT-M0 실측 15가 열어 둔 압축기 교체 카드를
이번에도 안 썼고, 그 판단이 이제 잰 값 위에 선다.

### 실측 33. 음성 확인 둘 — 검사 1이 또 tautology였다

| 무엇을 되돌렸나 | 검사 1(정적) | 화면 검사 |
|---|---|---|
| `usr/bin/fd` → `usr/bin/fdfind` | 초록 | 검사 9 FAIL — `fish: Unknown command: fd` |
| `copy_lib_deps`를 eza·bat 둘에만 건너뛰기 | 초록 | 검사 8 FAIL — `eza: error while loading shared libraries: libgit2.so.1.9` |

M1의 실측 24가 그대로 재현됐다 — 목록과 검사가 같은 배열을 읽으므로,
dest를 `fdfind`로 바꾸면 검사 1은 `usr/bin/fdfind`를 찾고 그것이 거기 있다.
정적 검사는 "목록이 완전한가"를 영원히 답하지 못한다.

둘째 음성 확인에서 검사 1~7이 전부 초록으로 지나갔다. M1의 실측 25에서는
`copy_lib_deps`를 통째로 빼서 셸 셋까지 죽었는데, 이번엔 겨냥한 둘만
건너뛰어 실패 반경이 정확히 그 둘이었다. "특정 도구를 겨냥한 음성 확인은
그 도구만 건너뛰게 해야 한다"가 M1이 남긴 처방이고, 그대로 따르니 그대로
됐다.

### 실측 34. 계획 밖 1 — QEMU의 기본 메모리 128MiB에서 기계가 안 켜진다

첫 체인 실행이 도구가 아니라 부팅에서 죽었다.
...
코드와 예약이 48MB라 128MiB 안에 자리가 없다. 넷을 재서 경계를 찾았다:

```
128M  panic (available 80,964K)     256M  뜬다 (209,716K)
512M  뜬다                          1024M 뜬다
```

이것이 UT-M0 실측 15의 반대편이다. 그때 78MB 밸러스트로 부팅해서 "크기의
벽이 없다"를 봤는데, 그 스파이크도 128MiB에서 돌았다. 78은 되고 84는 안
되는 경계 위를 지나온 것이고, 아무도 그 여유를 잰 적이 없다.

처방은 `gate_lib.sh`의 `GUEST_MEM=512`다. 512를 고른 것은 256이 뜨긴
하지만 tmpfs 84MB를 빼면 여유가 125MB뿐이고 UT-M3이 git과 vim.tiny를
더하기 때문이다.

이 수를 한 자리에 둔 것이 이 항목의 본체다. QEMU 호출이 저장소에 열둘
있고 전부 같은 initrd를 부팅한다 — `machine/check.sh`만 RM 때부터 `-m 512`를
손으로 갖고 있었고 나머지 열하나는 QEMU의 기본값으로 돌고 있었다. 아무도
그 수를 고른 적이 없다는 것이 UT-M2 전까지 드러나지 않았다. `boot/check.sh`와
`device/check.sh`는 타이핑을 안 하는데도 이제 `gate_lib.sh`를 source한다 —
이 수 하나 때문이다.

실기에는 영향이 없다. 128MiB는 QEMU의 기본값이지 이 기계의 요구사항이
아니다. 게이트만의 제약이었고, 그래서 실기에서는 영원히 안 보였을 것이다.

### 실측 35. 계획 밖 2 — `sleep 2`가 짐작이었고 8회 중 2회 틀렸다

메모리를 고친 뒤 체인이 한 번 통과하고 한 번 깨졌다. 깨진 자리가 `fd`였다가
`eza`였다가 해서 도구 문제로 보였는데, 깨진 회차의 마지막 화면에 찾던
글자가 정확히 찍혀 있었다.
...
돌려 6/8을 쟀다 — 게이트가 체인마다 3회 연속을 요구하므로 그대로 두면
루트 게이트가 거의 확실히 빨개진다.

원인은 `ps ax`다. 화면을 통째로 채운 뒤로 격자 전체를 다시 그려야 하고
(RC-M0: 한 프레임의 84.7%가 `fill`), 게이트는 arm64 호스트에서 x86_64를
TCG로 흉내내는 중이라 그 한 프레임이 2초를 넘는 회차가 있다. `fd` 하나만
따로 여덟 번 치면 8/8이다 — 앞의 `ps ax`가 있어야 재현된다.

처방은 `gate_lib.sh`의 `wait_for_screen`이다. 15초까지 0.1초 간격으로 다시
보고, 찾으면 즉시 돌아온다. `type_keys`가 GL-M2에서 배운 것과 글자 그대로
같은 교훈이고(고정 sleep은 짐작이다), 값도 같은 방향이다 — UT 체인의 고정
sleep 합계 17초가 부팅마다 사라졌으므로 이 변경은 게이트를 느리게 하지 않고
빠르게 한다. 2초를 넘게 기다린 회차는 말해 준다:
`the screen took about 3s to show /fonts/`.

나머지 열 체인은 아직 고정 sleep이다. 깨지는 것을 본 자리만 고쳤다 —
그쪽은 3/3을 여러 판 지나왔고, 다음에 깨지는 체인이 있으면 그 체인이 이
함수를 쓰면 된다. 이것이 RM-M0의 `-vga none`과 같은 모양이다: 전제를
암묵에서 명시로 옮기되, 안 깨진 자리는 안 건드린다.

### 실측 36. 부팅은 안 느려졌다 — 실측 15가 재현됐다

initrd를 2.4배로 키우고도 프롬프트까지 2.24~2.45초다.

```
Freeing initrd memory: 31260K
[    1.37] Run /init as init process
boot 1: 2.45s   boot 2: 2.24s   boot 3: 2.24s
```

압축기 교체 카드를 이번에도 안 썼다(`CONFIG_RD_ZSTD`가 켜져 있고
컨테이너에 `zstd`가 있다). 비용의 자리가 압축도 부팅도 아니라는 것이
두 번째로 확인됐다.

다만 실측 15가 못 본 것이 있었다 — 그때 "78MB로도 뜬다"를 보면서 남은
여유를 안 쟀다. 84MB에서 안 뜬다는 것이 그 여유가 6MB였다는 뜻이다.
크기의 벽은 부팅 속도가 아니라 RAM에 있었고, 그것이 실측 34다.

### 실측 37. 게이트 — 열한 체인 3/3으로 23분 02.73초

```
UT-M2 PASS: 3/3 consecutive runs succeeded
TARS check PASS: all chains 3/3 consecutive runs succeeded
docker run ... bash check.sh   0.23s user 0.51s system 0% cpu 23:02.73 total
```

UT-M1의 21분 35.63초에서 +1분 27.10초다(RM-M3 시점 열 체인
20분 29.84초에서는 누적 +2분 32.89초). 설명되는 값이다 — initrd가 gzip으로
18.6MB 커졌고 게이트가 회차마다 39번 부팅한다. 잡음 ±3분 안이다.

`wait_for_screen`이 게이트 안에서는 한 번도 2초를 안 넘었다 — 스물네 번의
기다림에서 `the screen took about` 줄이 0회다. 직접 재현에서 8회 중 2회가
깨졌던 것과 모순이 아니다: 그 회차들이 필요로 한 시간이 2초를 조금 넘는
것이었고, 게이트는 체인을 하나씩 돌려 기계가 덜 바쁘다. 고침이 필요 없었다는
뜻이 아니라, 고침이 있으면 이 차이가 보이지 않는다는 뜻이다.

위험 4가 이번에도 아무 체인도 안 건드렸다 — 도구 열셋과 라이브러리
열아홉을 더하고도 나머지 열 체인의 판정이 한 글자도 안 갈렸다. 갈린 것은
판정이 아니라 QEMU 인자 한 줄이었다(실측 34).

---

## UT-M3이 실행으로 증명한 것 — 다시 조사하지 말 것

2026-09-11 네 번째 세션. 목록이 63에서 65가 되고 링크 넷이 섰다.
이번 milestone의 성격이 앞의 셋과 다르다 — 더한 것의 대부분이 바이너리가
아니라 이름과 링크이고, 그 이름 둘은 우리가 고른 것이 아니라 git
바이너리가 컴파일 타임에 박아 둔 것이었다.

### 실측 38. 새 라이브러리가 0이다 — 이 문장이 처음 맞았다

착수 전에 `.deb` 둘을 풀고 `copy_lib_deps`와 같은 규칙으로 closure를
쟀다.
...
이미 데려와 있었다. 최종 목록의 "층 3 — 새 라이브러리 없음"이 M1·M2에서는
두 번 다 틀렸고 여기서 처음 맞았다(실측 22·30). 그래도 재고 나서
알았다 — 맞는 날과 틀린 날은 재기 전에 구별되지 않고, 이 확인은 30초다.

### 실측 39. `/usr/lib/git-core`는 안 넣어도 된다 — 실측 7을 정정한다

실측 7은 *"168개 중 대부분은 `git` 자신에 대한 하드링크"*라고 적었다.
trixie 2.47.3에서 실제로는 심볼릭 링크 141 + 실체 26(합 25.5MB)이고,
실체 26 중 큰 것 일곱 약 16MB가 결정 5가 뺀 네트워크 헬퍼다.
결론은 안 바뀐다. 그리고 한 걸음 더 갔다 — 그 트리를 통째로 안 넣고도
`init`·`add`·`commit`·`log`·`status`가 게스트에서 전부 돌았다. 전부 `git`
바이너리 안의 builtin이다.

### 실측 40. 신원 — `user.email` 하나가 필수다
...
없다 — `/etc/passwd`의 gecos에서 `root`가 오고 그것은 implicit이어도
허용된다. UT-M0이 놓은 뼈대가 여기서 값을 낸다(실측 2의 표가 "git이
커밋 작성자를 여기서 유추하려다 실패한다"고 적은 자리다 — 절반은 유추하고
절반은 설정이 필요하다는 것이 실제 답이었다).

그래서 커밋의 작성자가 `root <tars>`다. 게이트의 검사 15가 그 한 줄로
뼈대와 결정 8을 함께 본다.
...
`HOME=/` → `/.gitconfig` → `config/gitconfig`. git이 링크를 풀고 저쪽에
락을 잡아 쓴다 — 링크 자체를 덮어썼으면 설정은 tmpfs에 남았을 것이고,
화면에서는 둘이 구별되지 않는다. 읽는 자리를 바꿔서(`/config/gitconfig`를
cat) 물어야 갈린다.

게이트에는 설정 디스크가 없다. 그래서 이 검사가 증명하는 것은 "링크가
풀려 `/config`에 쓰인다"까지이고, 그 디스크가 부팅 사이에 파일을 지킨다는
것은 CP 체인이 `tars.conf`로 이미 증명한 같은 마운트의 성질이다.
알고 두는 경계다.

### 실측 42. Debian git은 페이저와 편집기를 alternatives 이름으로 부른다
...
둘 다 Debian alternatives가 postinst에서 만드는 링크라 `dpkg -x`로 푼
sysroot에 없다. 없을 때 무슨 일이 나는지도 봤다.
...
매달리는 것이 아니라 죽는다. 그래서 게이트는 안 깨지고 사람만
깨진다 — `git log`·`git diff`·`git branch -a`가 전부 이 경로이고, `-m` 없는
`git commit`과 `git rebase -i`가 `editor` 경로다.

`mawk`→`awk`와 같은 종류(결정 4)인데 한 가지가 다르다 — 그쪽 이름은
우리가 골랐고 이쪽 이름은 바이너리가 정해 놓았다. 처방은 링크 둘이다
(`pager`→`less` · `editor`→`vim`). vim은 이제 이름이 셋이고 실체는
하나다.

### 실측 43. 템플릿 26,140바이트 — 넣는다

없으면 `git init`이 매번 `warning: templates not found in
/usr/share/git-core/templates`를 찍는다. btop 테마를 뺀 판단과 다른 이유는
쓰임이다(실측 31) — 저쪽은 안 쓰는 것이고 이쪽은 `git init`이 매번 쓴다.
개발용이라 부르는 기계가 `git init`마다 경고를 내는 것은 고장으로 보인다.

### 실측 44. 긴 출력의 첫 줄은 화면 프레임에 안 남는다

`vi --version`의 판정을 처음에 `VIM - Vi IMproved`로 잡았는데 화면 프레임
어디에도 그 글자가 없었다(0회). 출력 약 50줄이 한 번에 오고, 프레임이
그려질 때는 첫 줄이 이미 스크롤로 사라진 뒤다.

판정은 출력의 첫 줄이 아니라 마지막까지 남는 줄로 잡는다 — `vi --version`
의 마지막 줄인 `Linking: gcc`다. 이 저장소에 없던 교훈이고, `dmesg`를 안
치는 이유(실측 26 — 출력이 화면을 뒤덮는다)의 뒷면이다. 저쪽은 뒤덮여서
안 치고, 이쪽은 뒤덮이는 것을 알고 뒤에 남는 것을 본다.

### 실측 45. sendkey에는 대문자가 없다

`git var GIT_PAGER`를 치니 변수 이름이 통째로 안 쳐졌다(화면에
`git var  `). QEMU monitor의 `sendkey`는 키 이름을 받으므로 대문자는
`shift-g`처럼 보내야 하고, 이 저장소의 체인은 그것을 쓴 적이 없다.
게이트가 칠 명령은 전부 소문자여야 한다 — 위 `git var -l`로 돌아가서
물은 것이 그 때문이다.

### 실측 46. 검사 1에 잠복 결함이 있었다 — 마지막 항목을 영영 못 찾는다

`.gitconfig`을 검사에 더하니 파일이 initrd에 분명히 있는데 FAIL이었다.

```
FAIL: .gitconfig is missing from the initrd
445:.gitconfig$          ← cpio 목록의 마지막 줄
```

원인은 패딩이다. `PADDED_LIST="$(printf '\n%s\n' "$INITRD_LIST")"`에서
명령 치환이 끝의 개행을 도로 지운다. 그래서 `*$'\n'"$want"$'\n'*`는
아카이브의 마지막 항목에 대해 언제나 거짓이다.

지금까지 안 드러난 이유는 마지막 항목이 한 번도 `WANT`에 없었기
때문이다. 증상이 조용한 초록이 아니라 설명 안 되는 빨강이라는 점에서
TR-M2의 글로브와 방향이 반대이고, 그래서 UT-M3의 첫 실행에서 바로 드러났다.
고침은 명령 치환을 안 쓰는 것이다: `PADDED_LIST=$'\n'"${INITRD_LIST}"$'\n'`.

### 실측 47. 크기와 시간

```
initrd gzip   32,007,022 → 34,855,376 bytes   (+2,848,354)
initrd plain  84,417,024 → 90,291,712 bytes   (+5,874,688)
usr/bin 항목            63 → 65  (+ 링크 셋)
새 라이브러리                    0
make_initrd.sh 전체       3.449 → 3.656초
프롬프트까지              약 3초 (1초 해상도)
```

`GUEST_MEM=512`가 이 수를 미리 보고 고른 값이었다(실측 34). 푼 것이
90MB이고 512MiB 안이라 여유가 넉넉하다 — 256을 골랐으면 여유가 165MB로
줄었을 것이고, 그 선택의 값이 한 milestone 뒤에 확인된 셈이다.
압축기 교체 카드는 이번에도 안 썼다 — 세 번째다.

### 실측 48. 음성 확인 셋 — 이번엔 정적 검사가 tautology가 아니었다

| 무엇을 되돌렸나 | 검사 1(정적) | 화면 검사 |
|---|---|---|
| `.gitconfig` 링크를 지웠다 | FAIL — 부팅 전에 죽는다 | (거기까지 못 간다) |
| `pager` 링크를 지웠다(+ literal도) | 초록 | 전부 초록 — 아무도 못 본다 |
| 목록에서 `usr/bin/git` 줄을 지웠다 | 초록 | 검사 12 FAIL — `fish: Unknown command: git` |

첫 줄이 M1·M2와 다르다. 그때는 정적 검사가 배열을 되읽으므로 무엇을
망가뜨려도 초록이었는데(실측 24·33), 링크 넷과 템플릿은 배열이 아니라
`tools/check.sh`에 literal로 적혀 있다 — 그래서 `make_initrd.sh`에서 그
줄을 지우면 부팅 20초를 쓰기 전에 죽는다. 목록과 검사가 같은 파일을 보면
tautology가 되고, 다른 파일을 보면 진짜 검사가 된다는 것이 두 줄로 나란히
보인 자리다.

둘째 줄이 가장 중요하다. `pager` 링크는 게이트가 영영 못 본다 —
게이트는 `--no-pager`로 치기 때문이다. 사람이 치는 모양과 게이트가 치는
모양이 다른 유일한 자리이고, 그래서 그 이름을 정적 검사에 literal로
박아 뒀다. 게이트가 그것에 대해 할 수 있는 전부다.

### 실측 49. 게이트 — 열한 체인 3/3으로 23분 43.15초

```
UT-M3 PASS: 3/3 consecutive runs succeeded
TARS check PASS: all chains 3/3 consecutive runs succeeded
docker run ... bash check.sh   0.23s user 0.52s system 0% cpu 23:43.15 total
```

UT-M2의 23분 02.73초에서 +40.42초다(RM-M3 시점 열 체인 20분 29.84초
에서는 누적 +3분 13.31초). 설명되는 값이다 — initrd가 gzip으로 2.85MB
커졌고(회차마다 39번 부팅한다) UT 체인이 타이핑을 약 150글자 더했다. 잡음
±3분 안이다. 위험 2가 "언제부터 느려졌나를 캘 수 있게 남긴다"고 적은 자리다.

`wait_for_screen`이 이번에도 한 번도 2초를 안 넘었다 — 기다림이 열다섯으로
늘었는데 `the screen took about` 줄이 0회다(M2와 같다). 고정 `sleep 2`로
뒀으면 UT 체인의 부팅당 대기가 30초였을 것이고, 지금은 화면이 준비되는 즉시
지나간다.

위험 4가 이번에도 아무 체인도 안 건드렸다. 도구 둘과 링크 넷을 더하고도
나머지 열 체인의 판정이 한 글자도 안 갈렸다 — fish 프롬프트가 저장소 안에서
`(main)`을 붙이는데도 그렇다(실측 10). 프롬프트 폭에 기대는 자리는
`copy/check.sh`의 `col 20` 하나뿐이고 CM 체인은 저장소 안으로 들어가지
않는다. UT-M0에서 `/etc/passwd` 한 줄이 그 수를 16에서 20으로 깬 것과
대조된다 — 그때는 모든 프롬프트가 갈렸고 이번엔 한 체인의 한 디렉터리
안에서만 갈린다.
