...
**23분 02.73초**였다 — M3 뒤의 값은 아래 실측 49에 있다.
...
> 안 본 것**이 원인이다. 크기 예측은 거의 맞았고 라이브러리 개수는 계속
> 모자랐다는 것이 이 항목의 요지다.
...
2026-09-11 세 번째 세션. 목록이 50에서 **63**이 됐고, `make_initrd.sh`는
...
initrd를 2.4배로 키우고도 프롬프트까지 **2.24~2.45초**다.
...
크기의 벽은 부팅 **속도**가 아니라 **RAM**에 있었고, 그것이 실측 34다.
...
UT-M1의 **21분 35.63초**에서 **+1분 27.10초**다(RM-M3 시점 열 체인
...
2026-09-11 네 번째 세션. 목록이 63에서 **65**가 되고 **링크 넷**이 섰다.
이번 milestone의 성격이 앞의 셋과 다르다 — **더한 것의 대부분이 바이너리가
아니라 이름과 링크**이고, 그 이름 둘은 **우리가 고른 것이 아니라 git
바이너리가 컴파일 타임에 박아 둔 것**이었다.

### 실측 38. **새 라이브러리가 0이다** — 이 문장이 처음 맞았다

착수 전에 `.deb` 둘을 풀고 `copy_lib_deps`와 **같은 규칙**으로 closure를
쟀다.
...
것은 **CP 체인이 `tars.conf`로 이미 증명한 같은 마운트의 성질**이다.
**알고 두는 경계다.**
...
치는 이유(실측 26 — 출력이 화면을 뒤덮는다)의 **뒷면**이다. 저쪽은 뒤덮여서
안 치고, 이쪽은 뒤덮이는 것을 알고 **뒤에 남는 것**을 본다.
...
**아카이브의 마지막 항목에 대해 언제나 거짓**이다.

**지금까지 안 드러난 이유는 마지막 항목이 한 번도 `WANT`에 없었기
때문이다.** 증상이 조용한 초록이 아니라 **설명 안 되는 빨강**이라는 점에서
...
UT-M2의 **23분 02.73초**에서 **+40.42초**다(RM-M3 시점 열 체인 20분 29.84초
...
늘었는데 `the screen took about` 줄이 **0회**다(M2와 같다). 고정 `sleep 2`로
