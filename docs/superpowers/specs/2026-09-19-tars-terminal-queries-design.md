...
Status: 끝났다(2026-09-19). M1 하나로 닫혔다 — 아래 "TQ-M1이 실행으로
증명한 것"이 그 결과다. 착수 전 조사는 ST-M3이 이 자리를 찾으면서 끝났고,
아래 "착수 전에 읽은 것"이 그것이다.
...
`terminal/check.sh`가 게스트에서 커서 위치를 묻고 답을 받아 길이를 찍는다.
정확한 문자열을 박지 않는 이유: 포맷은 라이브러리 몫이라 정확 문자열을 박으면
우리가 아니라 vendored 코드를 검사하게 된다. 답이 왔다는 사실이 우리 몫이다.

⚠ 구현하면서 셋이 달라졌다(M1 실측). (1) 질의 바이트를 게이트가 타이핑으로
만들 수 없다 — 이스케이프가 fish → bash → printf 층에서 죽는다. 그래서 ESC가
든 스크립트(`kernel/tq-probe.sh` → 게스트의 `/usr/bin/tq-probe`)를 initrd에
싣는다. (2) 묻기와 읽기가 그 스크립트 **한 프로세스** 안에 있어야 한다 —
답은 pty의 입력이라, 사이에 프롬프트로 돌아온 셸의 라인 편집기가 먼저
가져간다. (3) 판정 글자는 `R<n>`이 아니라 스크립트가 찍는 `len<n>`이고
행 경계 앵커(`\| `)를 붙이면 안 된다 — 답은 tty가 되울려(ECHOCTL이 ESC를
`^[`로 바꾼다) `^[[4;1Rlen6`처럼 한 행에 붙어 나온다.
...
1. **답이 화면에 새는가.** 답은 자식에게 가는 입력이라 자식이 raw 모드로
   읽으면 화면에 아무것도 안 남는다. ⚠ 다만 이 설계의 처음 문장("화면에 안
   그려진다")은 M1 실측으로 절반만 맞았다 — tty의 line discipline이 답을
   되울리고(ECHOCTL) 우리 terminal이 그걸 글자로 그려서, 게이트의 프로브처럼
   ECHO가 켜진 채로 읽으면 화면에 `^[[4;1R`이 남는다. fzf·less·vim은 자기
   상자를 그리기 전에 ECHO를 끄므로 그쪽에는 안 남는다. 게이트 판정 글자를
   그 되울림에 안 기대게 둔 이유가 이것이다(결정 5의 ⚠).
...
| | 무엇 | 판정 | 상태 |
|---|---|---|---|
| M1 | `effects.write_pty`를 채우고 답을 pty로 보낸다 · 단위 검사 · 게이트 검사 하나 · 씨앗 우회 제거 | terminal·config 체인 · 루트 게이트 | 끝났다(2026-09-19) |

## TQ-M1이 실행으로 증명한 것

M1은 2026-09-19에 끝났다. plan(`docs/superpowers/plans/2026-09-19-tars-terminal-queries-tq-m1.md`)의
Task 1~6을 그대로 밟았고, plan이 "이미 밟은 함정 둘"로 적어 둔 것 말고는 새로
막힌 자리가 없었다. 아래 아홉이 실행으로 확인된 값이다.

1. `effects.write_pty` 한 칸이면 답이 나온다. `vt_test`가 새 화면(20x5)에서
   `ESC[6n`(커서 위치)을 먹여 6바이트를, `ESC[5n`(상태 보고)을 먹여 4바이트를
   봤다. 두 검사가 함께 있어야 "커서 위치 하나만 특별 취급했다"가 안 통과한다.
   DA1(`ESC[c`)은 지금도 0바이트다 — 비목표 1이 그대로다.
2. 평범한 출력에는 답이 없다. `hello\r\n`을 먹인 뒤 버퍼가 0바이트다. 이
   검사가 없으면 "모든 출력마다 답을 만든다"가 위 둘을 그대로 통과하고,
   증상은 자식의 출력마다 자기 입력에 쓰레기가 섞이는 것이다.
3. 넘치면 통째로 버린다. 6바이트 답을 200번 몰아치니 85개(510바이트)가 남고
   115개를 버렸다 — 510 + 6 = 516 > 512라 다음 하나가 통째로 버려지는 계산과
   맞는다. 꺼낸 뒤 다시 꺼내면 0바이트다.
4. 게이트가 게스트에서 답을 봤다. 화면 원문은 이것이다 —
   `root@(none) ~# tq-probe | ^[[4;1Rlen6 | root@(none) ~#`. 커서가 4행
   1열(1부터 셈)에 있었으므로 답이 자리에 맞고, `len6`은 자식이 stdin에서
   읽은 글자 수라 답이 왔다는 것 말고는 설명이 안 된다.
5. 답은 tty가 되울린다. 위 원문의 `^[[4;1R`이 그 되울림이다(ECHOCTL이 ESC를
   `^[`로 바꾼다). 우리 terminal은 CPR 보고(`CSI Ps;Ps R`) 자체는 화면에 안
   그리는데, `^[`는 ECHOCTL이 만든 글자라 그려진다. 그래서 판정 글자를
   `len<n>`으로 두고 행 경계 앵커(`\| `)를 못 쓴다 — 계획은 `\| len[1-9]`였다.
   키를 raw 모드로 읽는 프로그램(fzf 등)은 ECHO를 꺼 두므로 그쪽에는 안
   남는다 — 되울림이 보이는 것은 게이트의 프로브처럼 ECHO가 켜진 채로 읽을
   때다.
6. 우회를 지운 뒤에도 picker가 첫 Ctrl+R에 뜬다 — config 체인 1차 부팅이
   초록이다. 그 부팅은 설정 디스크를 매 회차 새로 굽고(씨앗이 새로 깔린다)
   씨앗에서 `--no-height`가 사라졌으므로, fzf가 자기 높이를 정하려고 보낸
   질의가 답을 받았다는 뜻이다. 다만 높이 자체(40%)를 화면에서 잰 값은 없다 —
   "fzf 통합이 `--height 40%`를 준다"와 "그 답이 없으면 첫 누름에 멈춘다"가
   ST-M3의 실측이고, 그 사이가 이번에 초록이 됐다.
7. 프로브는 부팅마다 2초를 쓴다. `read -n 20 -t 2`는 6바이트만 오면 나머지
   14를 기다려 타임아웃을 꼭 채운다(bash로 잰 값: stdin이 열려 있고 2초 동안
   조용하면 2.001초. stdin이 닫혀 있으면 반대로 즉시 끝난다 — EOF다).
   terminal 체인이 이 검사 하나로 부팅당 2초를 더 쓴다 — `wait_for_screen`의
   15초 상한과는 무관하다.
8. 지운 것. 씨앗 셋의 `FZF_DEFAULT_OPTS` 줄과 그 주석, `config_test.zig`의
   `KNOWN_SEED_ENV`·`envs` 계상·그 역방향 검사다. 그 역방향 검사가 있던
   이유는 "그 줄을 지우는 편집이 조용해진다"였는데, 이제 지우는 것이 목적이라
   자리가 없어졌다. 설정 디스크를 이미 쓰던 기계는 그 줄을 그대로 갖고 있다
   (`O_EXCL`) — picker가 전체 화면으로 뜰 뿐 동작은 같다.
9. `tools/check.sh`의 `WANT`에 `usr/bin/tq-probe`를 더했다. 그 목록은
   `make_initrd.sh`가 손으로 넣는 파일의 자리다(`bin/sh`·`vi`·`pager`·`.bashrc`
   와 같은 줄) — 거기 적어야 그 줄을 지웠을 때 부팅(20초 + 빌드)이 아니라
   정적 검사에서 먼저 드러난다.
10. 루트 게이트 12체인 3/3 통과(`TARS check PASS`, `FAIL` 0줄, 34분 09초).
    그 판은 게이트의 `clean`이 커널을 처음부터 다시 빌드해서 기준선 16분과
    견줄 수 없다 — 이 변경으로 게이트가 길어졌다는 근거는 아니다.
