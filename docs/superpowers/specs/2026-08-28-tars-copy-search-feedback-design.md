...
Date: 2026-08-28
Status: CS-M0·CS-M1 완료(2026-08-28). 서브프로젝트 종료.
...
매치 목록이 서로 다른 객체가 된다는 것이다. 둘이 어긋나면 증상이 "`n`을
...
해제 자리는 `find`와 정확히 같은 셋이다 — `findSubmit`의 옛것 정리 ·
...
처음 구현은 `matches()`를 `findStep` 앞에서 불렀고, 그래서 `cells()`가 읽을
...
CS-M1 구현이 이 결정을 한 자리 고쳤다(2026-08-28). "`matches == 0`이면
켠다"를 그대로 쓰면 끄는 자리가 `main.zig` 하나뿐이 되어, 성공한 검색이
앞의 실패를 안 지우는 경로가 남는다 — poll 루프를 안 거치는 호출자(`vt_test`)가
그렇다. 그래서 `findSubmit`이 끝에서 `self.find_missed = count == 0;`으로
매번 값을 정한다. 켜는 자리와 끄는 자리를 안 가르는 것이 요점이고, 결정 6의
`refreshMatches`를 셋 다에서 부른 것과 같은 규율이다.

메시지에 쓸 글자는 `find_last`에서 온다. 메시지가 뜰 때는 프롬프트가 이미
닫혀 있어서 `findNeedle()`이 null을 주기 때문이다 — 결정 8과 9가 맞물리는
자리이고, 둘을 따로 만들 수 없다는 뜻이다.
...
그 선이 실제로 지켜졌다(2026-08-28). CS-M1은 `cells()`도 `RowSpan`도
`MATCH_BG`도 한 글자 안 건드렸고, CS-M0은 오버레이 문자열을 안 건드렸다.
두 milestone이 겹친 파일은 `vt.zig` 하나인데 겹친 함수가 `copyExit` 하나뿐
이고, 거기서도 CS-M0은 `find_matches`·`hl_spans`를 버리고 CS-M1은
`find_missed`를 끈다.
...
CS-M1의 plan은 CS-M0이 끝난 뒤에 썼다(저장소 규칙):
`docs/superpowers/plans/2026-08-28-tars-copy-search-feedback-cs-m1.md`
...
CN-M1의 `searchAll()`이 60~70밀리초였고 사람이 못 느꼈다는 기준선이 있다 —
...
이미 검사 15가 친다) — 새 검사는 같은 부팅에서 `style>` 줄을 더 보는 것에
가깝다.
