...
증명한 것을 담는다. 게이트 아홉 체인 3/3으로 **19분 40.02초**다(직전 18분
...
돌리는데 `PASS`는 **넷**이다 — `status_test.zig`만 그 줄을 안 찍는다(IS-M1이
...
11(`Cmd+V`가 모드 **밖**에서 — `chord()` 쪽)과 검사 12(모드 **안**에서도 —
...
뽑아 `put`과 **같은지**만 보므로 뜻("통째로 들어갔다")은 그대로다.
...
`input.zig`에 **한 줄**뿐이다(1249행). 결정 1이 말한 2 → 1이 코드에서 참이다.
