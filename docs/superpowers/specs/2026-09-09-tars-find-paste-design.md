...
**참고: 한 줄 yank는 개행이 안 붙는다.** 게이트 로그가
...
| 검사 | 무엇을 본다 |
|---|---|
| 55 | 한 줄 클립보드가 **통째로** 들어간다 |
| 56 | 여러 줄이면 **첫 줄만**(`가나\n다라` → needle이 `가나`) |
| 57 | 이미 친 글자 **뒤에** 붙는다(`/가` + `나` → `가나`) |
| 58 | 빈 클립보드는 `0`이고 needle이 **안 바뀐다** |
...
판정이 둘이다 — `zig build test`, 그리고 **`copy/check.sh` 한 체인.**
기존 붙여넣기 검사 셋(`copy/check.sh:391`·`446`·`495~507`)이 **한 글자도 안
바뀐 채 통과하는 것**이 "동작이 안 바뀌었다"의 증거다. 전체 게이트를 안
돌리고 한 체인만 돌린다.
...
| 줄 | 무엇 |
|---|---|
| `find> paste clip=3 put=3` | 클립보드가 needle에 닿았고 잘리지 않았다 |
| `find> overlay text=/가` | **기존 줄을 재사용한다** — needle이 그 글자다 |
| `find> submit … matches≥1` | 붙인 한글이 화면의 한글을 찾았다 |
| `key_lines`가 **안 늘었다** | **셸로 한 바이트도 안 샜다** |

**마지막 음성 검사가 이 설계의 유일한 위험을 정면으로 본다** — 판단을
끌어올린 대가가 "프롬프트에서도 셸로 쓴다"인데, `key>` 줄은 PTY로 나갈 때만
찍히므로 개수가 안 느는 것이 곧 안 샜다는 뜻이다. 검사 18이 이미 쓰는
수법이다.
