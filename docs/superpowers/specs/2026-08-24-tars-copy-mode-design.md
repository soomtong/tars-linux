...
`screens.active`는 이미 포인터다(`&`를 붙이면 `**Screen`이 되어 컴파일이
막힌다).
...
### 결정 3. 모드 안에서는 어떤 키도 PTY로 나가지 않는 것이 기본값이다
...
`vt.Screen`이 드는 것은 뷰포트 좌표의 커서 하나다. 앵커는 따로 안 든다 —
...
시나리오는 이렇다. 복사 대상을 `echo echo PASTED`의 출력 줄로 만드는
...
> 51분 20초다. 직전 기준선 45분 41초에서 5분 39초가 늘었으니 CM 체인
...
> 54분 40초다. 직전 기준선 51분 20초에서 3분 20초가 늘었는데, 그중 약
...
>    것으로 바꿨다. 전체 행 수가 줄었는지로 보는 안은 버렸다 — 한 번의
...
> 에코가 아니라 백그라운드 출력이다(`main.zig`의 그 자리 주석: 한 줄만
...
> assertion에 앱 이름이 찍혀 있었기 때문이고, 이번에는 그런 이름이 없었다.
>
...
>    걸렸다. `Cmd+V`가 바이트에서 copy 명령으로 의미가 바뀐 것을 보던
...
> 앵커의 screen 좌표 y를 비교하는 것으로 바뀌었다 — 위 CM-M1 블록의 1번.
