...
**4. QEMU `sendkey`의 세 키 조합.** `meta_l-shift-c` 형태가 게스트에
도착하는지 확인하지 않았다. IP 체인이 `meta_l-left` 두 키 조합을 쓰고 있으니
가능성은 높지만, CM-M0의 첫 실측 대상이다. 안 되면 진입키를 두 키 조합으로
바꾼다.

**5. 게이트 시간이 는다.** 새 체인은 부팅 3회를 더한다. 직전 기준선은 45분
41초이고(2026-08-24, 한가한 기계), 증가분은 CM-M0에서 실측한다. **값이
기준선에서 크게 벗어나면 코드를 의심하기 전에 기계를 먼저 의심한다.**
