...
> 기계 쪽에서 최소 2분 45초가 빠진 셈이다. `pmset -g log`를 보니 측정 구간에
> `coreaudiod` 오디오 컨텍스트가 여럿 열려 있었고 `Amphetamine`과
> `caffeinate`도 떠 있었다 — **이번 값도 깨끗한 기계에서 잰 것이 아니다.**
> CM-M1이 미설명으로 남긴 2분 20초와 방향은 일관되지만, 두 측정 모두 배경이
> 통제되지 않았으므로 비교 자체에 잡음이 섞여 있다. **이 게이트의 시간은
> ±3분 수준의 잡음을 가진 값으로 다루는 것이 맞다.**
>
> **설계가 실측에서 배운 것 하나.**
>
