...
2. 비어 있는 정상 경로를 채운다. Config Persistence가 정한 정책은
   "설정을 고치고 재부팅해야 반영된다"인데, 지금 TARS에는 게스트
...
1. `kill(-1, SIGTERM)`. PID 1이 부르는 `-1`은 자기를 제외한 모든
   프로세스를 뜻한다. 감독 대상 둘뿐 아니라 PTY 안에서 도는 셸까지 한 번에
...
검사는 `Attempted to kill init`이 없어야 한다는 것이다. 이것이 필요한
...
- `tars-init: started terminal` 줄이 정확히 세 개다.
