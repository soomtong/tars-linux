...
없어졌다. `HANDOFF.md`가 남긴 후보 셋 중 스크롤백·색상 렌더링을 골랐다.
...
- 스크롤백 크기를 지정하지 않는다. `vt.zig:42`가 `Terminal.init`에
  ...
  10,000바이트로 들어간다. 즉 스크롤백은 이미 쌓이고 있고 크기만 작다.
...
형식으로 이미 해소된 값이며, `Style`을 그대로 흘려보내지 않는다.
...
스크롤 키는 바이트가 아니라 동작이고 PTY로 새어 나가면 안 된다. 지금
...
> (`font_test`, 호스트), 게이트가 실제로 부팅해서 쓴 양은 1,784바이트다
