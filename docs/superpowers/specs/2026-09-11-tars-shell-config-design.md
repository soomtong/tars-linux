...
**Date:** 2026-09-11
**Status:** **SC-M0 완료**(2026-09-11). 자리가 섰다 — `tars.conf`의 여섯째
키 `shell_config`가 셸의 no-config 플래그를 켜고 끄고, `/config`의 rc 셋이
링크로 홈에 이어졌다. **SC-M1(씨앗)·SC-M2(탈출로 둘)가 남았고, 그 plan은 각
milestone 시작 시점에 따로 쓴다.**
게이트는 열한 체인 3/3으로 **24분 08.79초**다(착수 전 기준선 23분 43.15초에서
+25.64초, 잡음 ±3분 안).
