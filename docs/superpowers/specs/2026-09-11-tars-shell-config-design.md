...
**Date:** 2026-09-11
**Status:** **SC-M1 완료**(2026-09-11). 자리가 섰고(M0) 그 자리에 파일이
놓였다(M1) — `tars.conf`의 여섯째 키 `shell_config`가 셸의 no-config 플래그를
켜고 끄고, `/config`의 rc 셋이 링크로 홈에 이어지며, **init이 그 셋을 "없으면
만든다."** `config/check.sh`가 부팅 셋으로 늘어 **rc가 실제로 읽힌다**와
**`off`가 그것을 막는다**를 증명한다. **SC-M2(탈출로 둘)가 남았고, 그 plan은
그 시점에 따로 쓴다.**
게이트는 SC-M0에서 열한 체인 3/3으로 **24분 08.79초**였고, SC-M1은 실측 27에
있다.
...
(실측 22). **나머지 절반, 즉 "사용자가 쓰는 것"은 그대로 열려 있고 그것이
SC-M2다.**
