...
20분 23.41초, RM-M2 뒤 20분 15.37초, RM-M3 뒤 20분 29.84초다.
...
이것은 아직 아무 실기에서도 안 되는 것을 되게 하는 일이다.
...
바이너리지만 패키지는 아키텍처 중립이라, 컨테이너가 arm64인 채로
...
`RELOCATABLE`을 켜자 전체가 떴다. virtio-gpu를 하나도 안 물리고다.
...
기본은 1280x800이라 격자가 `155x47`이다. 새 체인의 판정은 이 수를 전제로
...
RC-M0이 재기만 하고 안 고친 `fill` 84.7%가 이 기계에서는 더 비싸다는 사실만
적어 둔다.
...
대가는 커널이 커지고 빌드가 느려지는 것이고 게이트가 그것을 15~18배로
...
대가는 시리얼 로그에 커서 이동 escape가 섞이는 것이다. limine이 글자마다
...
두 ISO로 나누지 않는다. `xorriso`의 El Torito는 부트 카탈로그에 항목 둘을
...
기능이 아니라 커널이 스스로 쓰는 방어이고, 끄는 것은 방어를 벗기는
편집이다. `project_kernel_config`의 규율("이해하는 것은 끄고 이해하지 못하는
것은 남긴다")에서 이것은 이해하지 못하는 쪽이다 — 부팅 시간에 얼마를 더하고
...
게이트가 못 보는 자리에서는 넓게 켠다는 것이 이 서브프로젝트의 기울기다.
...
- `/sys/block`을 훑기 — `getdents64`를 직접 다뤄야 한다. `devices.zig`가
  그 길을 일부러 피한 자리이고 근거를 적어 뒀다: "init에 libc도 힙도
...
것을 막는다. PID 1이 부팅 중에 거기서 멈추면 기계가 안 켜진다. 게이트가 못
보는 실패이고, 게이트가 못 보는 자리에서는 넓게 켠다는 결정 8의 기울기와
같은 방향이다.
...
→ 실현되지 않았다. 배수가 15가 아니라 1이고(실측 8) 증가분이
...
설정 변경이 옛 체인의 전제를 바꿔서다. 결정 2가 막으려던 위험(체인들을
...
게이트 로그를 세어 보면 실제 빌드가 1회이고 `kernel: bzImage matches
.config and build.sh, skipping make`가 29회다(체인 10 × 3회 = 30번 호출).
...
직전 아홉 체인 값이 19분 40.02초였으므로 +12.05초다. 체인 하나가 3회
...
한 줄도 안 적었는데 켜진 것 다섯이 층이 제대로 접혔다는 증거다.
...
RM-M0 뒤 값(19분 52.07초)에서 +31.34초다. 설명되는 값이다 — 커널 빌드
+5.68초(1배, 실측 8·11)와 체인이 새로 하는 일(모니터 연결 · 글자 셋 타이핑
0.4초씩 · 확인 대기 2초 · NVMe와 xHCI 초기화)이 3회 돈다.

착수 전 아홉 체인 값 19분 40.02초에서 보면 전체 +43.39초이고, 그 사이에
게이트가 부팅 33회에서 36회로 늘고 커널이 22.1% 커졌다.

### 실측 14. "장치가 보인다"와 "키가 화면에 닿는다"가 다르다

체인의 판정 여섯은 전부 커널이 만든 줄이거나 `init`이 연 결과다. 일곱째만이
USB 키보드로 친 글자가 PTY를 지나 격자에 그려지는가를 본다.
...
그 가짜를 만들 때 쓰는 오프셋이 구현이 쓰는 것과 같은 수다. 즉 1080과
...
Task 0이 오프셋을 보고, `machine/check.sh`가 셋이 함께 도는가를 본다.
...
디스크는 붙었고 커널도 봤는데 `init`이 라벨을 못 찾았다가 세 줄에 다 있다.
...
RM-M1 뒤 값(20분 23.41초)에서 −8.04초다. 잡음(±3분) 안이고, 커널이 안
...
RM-M1부터 잠복해 있던 경합을 게이트 위로 밀어 올렸다 — `init`이 USB
...
줄로 하고, 그 둘이 증명하는 것은 "코드가 들어갔고 init이 돌았다"까지라고
체인 주석에 적었다.
...
뒤로 밀려 우연히 경합을 피한 것이지, 고침이 동작한 증거가 아니다.
...
RM-M2 뒤 값(20분 15.37초)에서 +14.47초다. 잡음(±3분) 안이고, 커널이
...
`device/check.sh`가 그 줄의 부재를 요구하는 것이 여기서 값을 낸다.
...
사용자가")에 대한 이 서브프로젝트 한정 예외이고 SH·FP 세션의 예외와 같은
종류다.
