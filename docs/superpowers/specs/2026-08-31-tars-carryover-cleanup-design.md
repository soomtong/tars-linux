...
### 3. 우리 빌드는 vendor된 libghostty-vt 라이브러리를 안 쓴다

`terminal/build.zig.zon`이 ghostty를 Zig 패키지로 잡고
...
`prepare.sh`를 돌려 `zig build`가 통과하는 것이 "빌드가 그것을 안 쓴다"의
증명이다(검산 3).
...
검산 다섯이 전부 통과했다. 게이트는 여덟 체인 3/3, 16분 48.91초다.
...
| 무엇 | 값 |
|---|---|
| bzImage | 2,946,048 → 2,933,760바이트 (−12,288, 0.42%) |
