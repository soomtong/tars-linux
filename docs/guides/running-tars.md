...
`make`나 게이트로 빌드한 산출물(`out/tars.iso`, `kernel/build/...`)을 로컬
화면·VM·실기 노트북에서 직접 띄워 보는 방법을 정리한다. 빌드 자체는
`README.md`를 본다.
...
버전도 다를 수 있다. 눈으로 보는 용도로 쓰고, 통과 여부는 게이트로 정한다
(`README.md`의 "게이트" 절).
...
커널에 넘긴 프레임버퍼 위의 simpledrm에서 나온다 — 실기 노트북이 밟는
...
디스크의 p2에 살고, 이 스틱은 필요 없다. 꽂아 두면 p2 대신 이 스틱이 붙는다 —
디스크 전체를 파티션보다 먼저 훑기 때문이다. 노트북 내장 디스크의 GPT 자체는 이
...
도구 65개 중 대부분은 이름으로만 닿는다. 그중 eza 하나를 `ls` 자리에
앉혔다. 별칭 넷은 `ls`가 돌아가는 자리 셋(`/config/zshrc` ·
`/config/xdg/zoxide` · `/sys/class/net`)을 부팅으로 재서 통과시킨 뒤에
들어갔다 — 별칭은 그 이름을 치는 모든 자리에서 돌고, 설정 디스크를 붙이는
체인들은 rc 켜진 셸에 명령을 넣는다.

| 치는 것 | 무엇이 나오나 |
|---|---|
| `ls` | `eza`. 색이 있고 아이콘은 없다 — eza의 아이콘은 유니코드 사설 영역이라 이 화면의 폰트(unifont)에 글리프가 하나도 없고, 붙이면 빈 칸만 생긴다 |
| `ll` · `la` | 긴 목록 · 숨김 포함 긴 목록. 둘 다 디렉터리가 먼저 온다 |
| `lt` | 두 단계 트리. 더 깊이 보려면 `eza --tree --level=N` |

`cat`·`find`·`grep`·`sed`·`du`·`df`·`top`은 안 가린다. 앞의 셋은 `bat`·`fd`·
`rg`·`sd`와 플래그 뜻이 달라 조용히 다른 답을 내고, 뒤의 셋은 화면을 통째로
가져가는 TUI다.

### git에도 기본값이 있다

`/.gitconfig`가 가리키는 `/config/gitconfig`가 첫 부팅에 깔린다. 담긴 것은
여섯 줄 남짓이다 — 새 저장소의 기본 브랜치(`main`) · pager(`less -FRX`) ·
색(`auto`) · 별칭 넷(`git st` · `git lg` · `git co` · `git br`).

| 고치는 길 | 무엇 |
|---|---|
| `git config --global <키> <값>` | 링크를 통해 그 파일에 쓰인다 |
| `/config/gitconfig`를 직접 편집 | 같다 |

`-FRX` 중 `-X`가 요점이다. less가 alternate screen을 안 쓰면 `git log`의
출력이 화면의 스크롤백에 남고, 그 로그를 copy mode로 훑을 수 있다.

`[user]` 절은 없다 — 작성자를 git이 `/etc/passwd`에서 유도한다
