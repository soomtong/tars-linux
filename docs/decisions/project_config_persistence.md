...
3. 마운트 실패는 치명적이지 않다. BF·TF 체인은 `-drive`가 없어 매 부팅 이
   ...
   `O_CREAT`가 성공해 버리고, 그건 재부팅하면 사라지는 가짜 영속성이라
...
4. 바이너리 부재 폴백(CP-M2). 이름이 화이트리스트를 통과해도 initrd에 그
   ...
   부팅이 되고, 원인은 로그 깊숙한 execve 한 줄뿐이다.
...
- 게스트 안에서 타이핑한다. 호스트에서 `debugfs`로 이미지를 편집하는 쉬운
  ...
  아니라 키를 보내므로 `=`는 `equal`, `>`는 `shift-dot`, `/`는 `slash`이고,
...
- exec된 바이너리는 밖에서 볼 방법이 없다. 그래서 `init`이
  ...
  파싱 결과(`config shell=`)와 이 줄의 개수가 짝을 이루는지가 CP-M2의 증명
...
- zsh는 바이너리 하나가 아니다. 기능 대부분이 dlopen되는 `.so` 모듈이고
  ...
  라이브러리처럼 `/lib/x86_64-linux-gnu`로 모으면 안 되는 유일한 예외다.
...
처리·설정 스키마 버전은 의도적으로 비워둔 자리다. 관련:
