...
단어이고, vim의 `w`를 만들려면 공백 덩어리를 한 번 더 건너뛰는 일을 우리가
해야 한다.
...
눌러 col 11(`gamma`의 `g`)로 가면 yank 결과가 `"beta g"` 여섯 자다.
...
`handleKey`에서 `.find` 분기가 copy 표보다 앞이다. 그래서 같은 `n`이
...
variant 추가와 다른 커밋으로 했다 — 한 Step에 두면 컴파일 에러 목록에 둘이
...
매치는 넷이다 — `echo findme` 한 번이 셸이 되비춘 명령줄과 출력줄 두
줄을 남기기 때문이다.
