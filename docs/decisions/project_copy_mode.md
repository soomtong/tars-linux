...
**선행 조건이 셋이다. 하나도 지금 없다.**

1. **스크롤백** — `terminal/src/vt.zig`의 `Screen.cells`는 현재 화면
   (`state.rows` × `state.cols`)만 훑는다. 화면 밖으로 밀려난 줄에 접근하는
   경로가 없다. libghostty-vt는 스크롤백을 갖고 있으므로 막힌 곳은 우리 쪽
   조회 API다.
2. **선택 상태와 렌더링** — 선택 영역을 반전시켜 보여주려면 렌더러가 셀별
   속성을 그릴 수 있어야 한다. `terminal/src/main.zig`의 `drawGlyph`는
   `TEXT_COLOR` 상수 하나로 흑백만 그린다 — 색상/속성이 먼저다.
