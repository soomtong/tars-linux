...
요청 내용: 스크롤백이 생기면, 특정 키로 선택 모드에 들어가서 화면
...
그대로 남고 pin도 유효하며, 달라지는 것은 그것이 가리키는 내용뿐이다.
...
2026-08-15의 요청이 전부 구현됐다. 게이트가 왕복을 본다 — 화면에서 잡은
...
- `chord()`의 Meta 분기 — 모드 밖의 붙여넣기를 맡는다.
- copy 표 — 모드 안의 붙여넣기를 맡는다. copy 분기가 `chord()`보다 앞에
...
CM-M2에서 그것과 별개의 축이 드러났다. `Copy`에 `paste`를 더한 것 자체는
`input_test`를 안 깨뜨렸지만(`expectCopy`·`expectCtx`는 `Action`만 훑는다),
`Cmd+V`가 바이트에서 copy 명령으로 의미가 바뀐 것을 보던 IP 시절 검사
...
여러 줄을 붙이면 개행이 곧 실행이다. 셸이 그 모드를 받는지 확인한 적이
