...
판정 로직의 결과다. [[project_gate_chain_composition]]의 "게이트는 자기가 안
...
device /dev/input/event`가 있어야 한다와 `no keyboard found`가 없어야
한다를 둘 다 요구한다. `device/check.sh`도 같은 쌍을 요구하고, 전원 버튼에
...
정상이다. 허브 둘을 끼워 재 보니 열거가 1.693초였다 — 허브를 거친
...
바이너리가 커지며 훑는 시점이 뒤로 밀려 우연히 경합을 피한 것이다.
