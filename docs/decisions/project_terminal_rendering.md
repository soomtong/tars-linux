...
`yoff`는 글자마다 다르고 편차가 5픽셀이다(Hanme 실측: `A` -14, `g` -11,
`한` -16). 그래서 **`g`의 디센더가 화면에서 잘려 나가고 있었다.** 라틴만
있을 때는 편차가 3픽셀이라 눈에 덜 띄었다.
...
| | 값 | 잰 곳 |
|---|---|---|
| 완성형 11172자를 **전부** 구웠을 때 | 2,157,133바이트 (2.06MB) | `font_test`, 호스트 |
| 게이트가 실제 부팅에서 쓴 양 | **1,784바이트** | `terminal: font>`, 게스트 |

상한조차 128MB의 1.6%다. **lazy가 옳은 이유는 굽는 시간이다.** 11172자를
굽는 데 컨테이너의 arm64 native에서 29.3밀리초가 드는데, 게스트는 TCG라 그
몇십 배가 붙는다. 커널이 `/init`에 넘기는 시각이 1.12초인 기계에서
([[project_kernel_config]]) 부팅에 그만한 시간을 더할 이유가 없다.
