...
항목뿐이다. 프롬프트가 없으면 `olddefconfig`가 기본값으로 되돌린다.
...
- `DRM_XE` 항목이 사라졌다 — `depends on X86_PLATFORM_DEVICES ||
  ...
  의존이 무너져 항목째 없어졌다. 방향이 반대인 변화도 생긴다는 예시다.
...
## 빌드 시간은 15배로 곱해진다 — RM-M0(2026-09-09)에 1배가 됐다

> 이 절의 배수는 낡았다. GL-M0이 `clean()`을 게이트 시작 1회로 옮겼고
> GL-M1이 `.config`와 `build.sh`의 sha256 스탬프를 넣었다. RM-M0이 게이트
> 로그를 세어 확인했다 — 실제 커널 빌드 1회, `skipping make` 29회
> (체인 열 × 3회 = 30번 호출). 설정 변경이 빌드에 더하는 시간은 1배로만
> 치러진다. RM-M1의 노트북 장치들이 더한 5.68초가 게이트에서도 5.68초다.
>
> 아래 문단은 HD-M1 시점(2026-08-21)의 기록으로 남긴다 — 문서의 배수는
> 게이트 시간과 같은 종류로 낡는다는 것이 그 자체로 교훈이다.

루트 게이트의 `clean()`이 매 회차 `kernel/build`를 지우므로
(`check.sh:15`), 체인 다섯 × 3회 = 커널 빌드 15회다. 설정 변경이 빌드에
더하는 시간은 그대로 15배가 된다.
