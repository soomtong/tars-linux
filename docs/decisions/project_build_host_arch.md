...
arm64로 바꿨다. 게스트(TARS)는 여전히 x86_64다. 즉 이 저장소는 이제
전부 크로스 빌드이며, 컨테이너 안에서 x86_64 바이너리가 실행되는 일은
...
바이너리 자체가 Rosetta 에뮬레이션을 통과하고, 그 안에서 다시 게스트
...
고쳐야 한다는 것이다(`apt-get install` 한 줄로 안 끝난다). `apt-get
...
initrd 안 라이브러리 자리는 `/lib/x86_64-linux-gnu`로 고정한다. 옛
