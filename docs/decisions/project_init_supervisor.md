...
pid가 내 목록에 없는 것이 정상이다 — 그 경우 `reaped orphan pid N`만
...
`poll` 전환은 여섯 체인 전부가 딛고 선 코드를 건드린 것이다. 앞으로 이
...
- TF 체인의 `reaped orphan pid` — `WNOHANG`으로 바뀐 뒤에도 재부모화된
  고아를 거두는지 본다.
...
`tars-init: reaped orphan pid N`으로 수거가 실제로 도는 것을 관측할 수
