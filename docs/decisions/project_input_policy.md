...
관계가 있어서가 아니라 그것이 readline·zle의 기본 바인딩이기 때문이다.
후보였던 다른 방식(우리가 새 시퀀스를 정하고 셸 설정으로 받게 하는 것)을
버린 결정적 이유는 검증이다 — A안만이 `--no-config`/`--norc`/`-f`로 뜬
...
`std.mem.eql(u8, kb, "pc")` 한 줄로 받는다 — enum을 복사하면 그 순간
...
넘기는 argv를 `{shell_path, shell_flag}` 둘로 따로 조립하기 때문이다.
...
가로채는 자리는 `chord()`이고 Meta·Alt 분기보다 뒤다. 위 두 분기는 조합이
...
- 범용 키바인딩 엔진은 만들지 않았다. `keyboard=apple|pc`는 재배치가
  아니라 하드웨어 선언이다 — "스페이스 옆 두 키가 어느 순서인가"라는
