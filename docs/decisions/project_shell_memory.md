...
그것은 **전원이 끊긴 것**이고, 셸이 나갈 때 하는 일이 하나도 안 일어난다.
...
`fc -W`를 직접 치는 것**이다(되읽기 `wc -l`을 함께 둔다 — 없으면 8차가 빨간
...
**그 DB가 어느 디스크에 있는가**뿐이고, 그것은 기계를 한 번 꺼 봐야 드러난다.
...
나가면 아직 로그를 쏟던 `tr`이 SIGPIPE로 죽고, `pipefail`이 그 **141**을
...
더 파지 않았다 — **처방이 한 글자 옮기는 것**이라서다. ⚠ 이 실패는 **컴파일
에러와 구분이 안 되는 모양**으로 나오므로, `zig build`가 죽으면 **먼저 한 번
더 돌린다.**
