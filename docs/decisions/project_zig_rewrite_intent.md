...
2026-08-10 결정: TARS는 언젠가 Rust 컴포넌트를 전부 Zig로 재작성한다.
현재 Rust(`init/` = PID 1 `tars-init`, `kms/`)와 Zig(`terminal/`)를 혼용
중인데, 이 혼용은 의도된 아키텍처가 아니라 과도기 상태다.

동기는 성능이나 이식성이 아니라 Zig를 제대로 써보고 싶다는 학습 욕구다
...
막히는 것은 구조체가 아니라 `_IOR`/`_IOWR` 계열 매크로다. 재작성
...
Rust 구현을 참고해야 할 일이 생기면 작업 트리가 아니라 git 히스토리를
