...
프로세스의 환경변수는 리눅스 커널이 PID 1에게 준 두 개가 전부다.
...
`kernel/make_initrd.sh`가 `/usr/share/terminfo/x/xterm` 파일 하나를
...
How to apply: 게스트에 지금 있는 환경변수는 넷이다 —
...
1칸이다. 그러면 커서가 두 칸짜리 글자의 가운데에 서고 다음 글자가 앞
