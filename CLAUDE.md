...
- 완료된 서브프로젝트: Boot Foundation(BF-M0~M4) · Display Foundation(DF) ·
  ...
  **Shell Config(SC-M0~M1, 2026-09-11 완료 — 아직 진행 중인 서브프로젝트다.
  ...
  **씨앗에 쓸 수 있는 줄은 주석과 `alias` 둘뿐이고**(부팅할 때 한 글자라도
  ...
  `config/check.sh`가 **부팅 셋**으로 늘어 rc가 읽히는 것과 `off`가 그것을
  막는 것을 증명한다. **남은 것은 탈출로 둘(SC-M2)뿐이다.)** design은
  `.../specs/2026-09-11-tars-shell-config-design.md`, 기억은
  `docs/decisions/project_shell_config.md`.
  **SC-M0·M1도 편집을 Claude Code가 했다** — 사용자가 2026-09-11에 외출하며
  "이번 세션의 구현에 대한 모든 결정을 위임한다"고 두 세션 연속으로 정했다.
  UT의 네 세션과 같은 종류이고 **세션 단위**다.
