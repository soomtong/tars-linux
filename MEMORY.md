...
- [Guest environment](docs/decisions/project_guest_environment.md) — 게스트 환경변수는 커널이 준 `HOME=/`·`TERM=linux` 둘뿐: `PATH`가 없어 외부 명령은 절대 경로로, `TERM`은 거짓말 중이라 `terminal`이 `xterm`으로 덮어써야 한다(시리얼 셸은 `linux` 유지)
