...
- [Wired NIC](docs/decisions/project_wired_nic.md) — 노트북형 유선 드라이버를 켜고 모든 QEMU 호출이 `-nic none`/`-netdev`를 명시한다(`require_explicit_nic`); init은 `dhcpcd -j /dev/console`을 인자 없이 띄우기만 하고 인터페이스는 dhcpcd가 고른다 — manager mode는 lease 전에 배경으로 가서 `-j` 없으면 로그가 사라진다(WN-M0~M3, 2026-09-26 종료). 열네번째 체인 `nic/check.sh`
