...
#
# RM 체인은 **실머신 노트북으로 가는 길**을 본다. 아홉 체인과 갈리는 자리가
# 셋이다.
#   1. 펌웨어가 SeaBIOS가 아니라 UEFI(OVMF)다. 노트북은 예외 없이 UEFI다.
#   2. 픽셀이 virtio-gpu가 아니라 **EFI GOP 프레임버퍼 위의 simpledrm**에서
#      온다. DRM_I915도 AMDGPU도 안 켰다 — 안 켜도 된다는 것이 RM의 실측이다.
#   3. 키보드가 PS/2가 아니라 **USB HID**다. i8042=off로 PS/2를 아예 끄지
...
#
# UT-M1부터 그 목록이 **make_initrd.sh와 같은 파일**에서 온다
