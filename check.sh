...
#
# CP-M2부터는 그 1차 부팅에서 monitor sendkey로 게스트 셸에 직접 타이핑해
...
# 이 체인들 전부가 딛고 선 코드를 건드린 것이라, 앞으로 그 자리를
...
# 직접 되읽는다는 것이다 — 나머지는 전부 로그 문자열만 본다. 게스트에
...
# 기능의 가장 흔한 실패 방식이다.
#
# 여기만 쓰는 도구가 하나 있다: 반전된 셀의 개수다. 한글은 두 칸이라
...
#
# RM 체인은 실머신 노트북으로 가는 길을 본다. 아홉 체인과 갈리는 자리가
# 셋이다.
#   1. 펌웨어가 SeaBIOS가 아니라 UEFI(OVMF)다. 노트북은 예외 없이 UEFI다.
#   2. 픽셀이 virtio-gpu가 아니라 EFI GOP 프레임버퍼 위의 simpledrm에서
#      온다. DRM_I915도 AMDGPU도 안 켰다 — 안 켜도 된다는 것이 RM의 실측이다.
#   3. 키보드가 PS/2가 아니라 USB HID다. i8042=off로 PS/2를 아예 끄지
#      않으면 init이 그쪽을 먼저 골라서 USB 경로가 안 밟힌다.
#
# 부트로더를 실제로 지나는 두 체인 중 하나다(다른 하나가 BF). 나머지 여덟은
# -kernel로 limine을 통째로 건너뛴다.
#
# 회차당 부팅 1회라 총 부팅 횟수는 33회에서 36회가 된다.
#
# UT 체인은 게스트가 명령을 이름으로 찾는가를 본다. 열 체인과 갈리는
...
#
# UT-M1부터 그 목록이 make_initrd.sh와 같은 파일에서 온다
# (kernel/guest_tools.sh). 체인이 도구 이름을 다시 적지 않으므로 목록과
# 검사가 어긋날 자리가 없다 — 대신 그 검사는 "목록이 완전한가"는 못 본다.
# tools/check.sh의 검사 1 주석이 그 경계를 적어 뒀다.
#
...
#
# 우리 코드가 하는 일은 dhcpcd를 띄우는 로그 한 줄이 전부다(started dhcpcd).
# 처음에는 링크를 올리는 줄이 하나 더 있었는데 WN-M2가 그것도 dhcpcd에게
# 넘겼다. 인터페이스를 고르는 것도, 주소도 기본 경로도 /etc/resolv.conf도
# dhcpcd가 쓴다. 그래서 이 체인의 검사 대부분은 우리 코드가 아니라 그 경계가
# 제대로 그어졌는지를 본다.
#
