...
/// 경로다(실측 4).
///
/// 도구 둘이 없는 경로를 스스로 만든다는 것을 알고도 만드는 이유는
...
/// 이름은 화이트리스트가 막아주지만 initrd에 안 들어간 셸은 이름이
/// 맞아도 실행되지 않는다.
///
/// 이 함수가 없으면 그 상황이 이렇게 나타난다: execve가 127로 죽고, 감독
/// 루프가 1초 간격으로 세 번 재시작한 뒤 포기하고, 셸이 하나도 없는 부팅이
...
/// 제어 터미널 잡기는 PID 1이 아니라 콘솔 셸 자식이 한다. setsid()로 새
...
fn supervise(
    ...
    while (true) {
        ...
        // ── 거둘 것을 전부 거둔다 ────────────────────────────────────
        //
        // 이것이 poll보다 앞인 것이 backoff를 만든다. 자식이 죽으면 이
...
pub fn main(init: std.process.Init.Minimal) void {
    ...
    // 안 고쳐도 된다 — 게이트도 그 한 줄에서 실효값을 읽는다.
    //
    ...
    // 심는 것이 된다. 폴백 뒤의 값을 쓰면 로그의 `config shell=`과 여기가
    // 언제나 같은 셸을 말한다.
    //
    ...
    // 매번 죽고, `boot/check.sh`가 그 재시작 횟수를 정확히 3으로 세고
