# HANDOFF: Inbound Network(IN)가 끝났다

## 지금 어디인가

서브프로젝트 Inbound Network(IN)가 2026-09-14에 열렸고 같은 날 M0·M1·M2가
전부 끝났다. `net/check.sh`가 검사 열여섯이 됐고, 새 검사 다섯이 "게스트가
포트를 열면 바깥에서 붙어 바이트를 읽고, 반대로 보낸 것도 게스트에 닿고,
듣는 것이 없으면 그것이 빨간불이 된다"를 매번 확인한다. 다음 할 일은 다음
...
루트 게이트는 열두 체인 3/3이고 31분 51.46초다(기준선 30분 57.86초에서
+53.60초, `skipping make` 35회). IN이 저장소에서 바꾼 코드 파일은
`net/check.sh` 하나다 — M0은 한 글자도 안 바꿨고, 커널도 `guest_tools.sh`도
`init/`도 IN 내내 그대로다. 그 체인 단독이 22.911초 → 27.955초 → 33.730초로
갔고, 게이트가 그것을 세 번 도니 IN 전체의 예상 증가분이 약 32초인데 게이트
잡음이 ±3분이라 그 크기는 게이트에서 안 갈린다.
...
IN이 닫혔고 손에 든 일이 없다. 고르는 자리이고, 그 결정은 사용자의 것이다.

IN design이 비목표로 미뤄 두면서 "다시 열릴 조건"을 함께 적은 것이 넷이다.

- UDP. 사용자가 이번 범위에서 뺐다. TCP가 선 뒤로는 같은 이야기의 반복이고
  `hostfwd`의 형식만 다르다.
- 포트 여럿. 하나가 서면 둘도 선다. "무엇을 서빙할 것인가"가 정해진 뒤에
  그 필요에서 나와야 한다.
- init이 듣는 것. 지금 이 저장소의 코드에는 `bind`도 `listen`도 `accept`도
  한 번도 안 나온다. 다시 열릴 조건이 "게스트가 무엇을 서빙해야 하는지가
  정해지는 것"이다.
- 실머신에서 포트를 여는 것. `hostfwd`는 QEMU의 것이라 실기계에 없다.
  실기계는 NIC가 LAN에 붙는 것이 전제이고 그것이 NW 비목표 1이다.

그리고 NW를 고를 때 함께 놓였다가 안 고른 후보 셋이 그대로 남아 있다 —
패키지 매니저 · 실머신 NIC · NTP.
