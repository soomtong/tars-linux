# HANDOFF: Inbound Network(IN)의 M1까지 끝났다

## 지금 어디인가

서브프로젝트 Inbound Network(IN)가 2026-09-14에 열렸고 같은 날 M0과 M1이
끝났다. `net/check.sh`가 검사 열셋이 됐고, 새 검사 둘이 "게스트가 포트를 열고
바깥에서 붙어 바이트를 읽는다"를 매번 확인한다. 다음 할 일은 IN-M2 plan을
쓰는 것이다 — 반대 방향(검사 14)과 음성(검사 15), 그리고 게이트 한 판.

이 서브프로젝트가 답하는 질문은 "게스트가 포트를 열면 바깥에서 붙을 수
있는가" 하나다. 우리 코드는 0줄이다 — 듣는 것은 게스트의 `nc`이고 거는 것은
체인의 bash `/dev/tcp`이며, 우리가 더하는 것은 QEMU의 `hostfwd` 한 줄과
게이트의 검사 넷이다. 그래서 실패하면 원인이 커널이거나 QEMU이거나 게이트
자신이고, 셋이 서로 멀어서 잘 갈린다.

방향은 사용자가 골랐다. 후보 넷(패키지 매니저 · 실머신 NIC · NTP · 포트
열기)에서 포트 열기를, 목적으로는 "받는 길이 서는지를 증명"을, 듣는
주체로는 게스트의 도구를, 자리로는 "`net` 체인을 늘린다"를, 범위로는
"양방향과 음성 검사"를 정했다. design의 결정 1·4·5가 그 넷이다.

그 앞이 NW(게스트에 주소가 붙는 것)이고 2026-09-14에 닫혔다. `tars.conf`에
`net=dhcp`를 적은 부팅에서 게스트가 `10.0.2.15`를 받고, 기본 경로가 깔리고,
TCP로 상대에 붙어 글자를 읽는다. 체인 `net/check.sh`가 검사 열하나로 그
사슬을 보고 `CHAINS`의 열두번째다. IN은 그 체인과 그 부팅을 그대로 쓴다.

루트 게이트는 열두 체인 3/3이고 30분 57.86초다. IN-M0은 저장소의 코드 파일을
한 글자도 안 바꿨고, IN-M1은 `net/check.sh` 하나만 바꿨는데 그 체인 단독이
22.911초에서 27.955초가 됐다 — 게이트가 그것을 세 번 도니 예상 증가분이 약
15초이고 잡음(±3분) 안이다. 게이트는 IN-M2가 끝난 뒤에 한 번 돌린다.
...
- 포트는 45465·45466으로 정해져 있다(monitor 대역 45455~45464·45471과 안
  겹친다). 45465는 IN-M1이 이미 `net/check.sh`에 넣었고 45466이 M2 몫이다.
  `hostfwd` 둘을 한 `-netdev` 값에 쉼표로 이어 붙이는 것이 이 QEMU에서 그대로
  먹고, `hostfwd`와 `guestfwd`가 한 값에서 함께 사는 것도 확인됐다(실측 10).
- 판정 글자가 정해져 있다 — `inm1-inbound-ok`(게스트 → 체인)와
  `inm2-reverse-ok`(체인 → 게스트). design 결정 8이 근거다.
- 키 이름 둘이 이 저장소에서 IN-M1에 처음 쓰였고 실제로 돈다(실측 9) —
  `shift-backslash`가 `|`, `shift-7`이 `&`다.
...
## 바로 다음에 할 것 — IN-M2 plan을 쓴다

IN-M1이 끝났고 손에 든 일이 IN-M2다. design의 "Milestone 셋" 절에 M2의
윤곽이 있고, IN-M1 plan 끝의 "IN-M2가 이 자리에서 이어받을 것" 넷이 그 위에
구체적인 값을 얹어 두었다.

M2가 하는 일이 셋이다.

- 검사 14 — 반대 방향. `hostfwd` 둘째 줄(45466 → 게스트 8081)을 같은
  `-netdev` 값에 쉼표로 이어 붙이고, 둘째 리스너를
  `nc -l -p 8081 > /tmp/<파일> &`로 띄운 뒤 체인이 `inm2-reverse-ok`를 보내고
  바로 닫는다. 게스트에서 그 파일을 `cat`해서 화면으로 판정한다. 이 방향은
  체인이 읽을 바이트가 없어서 화면 판정이 불가피한데, 판정 글자를 우리가
  게스트에 타이핑하지 않으므로 에코 함정에 안 걸린다. 모양은 M0 실측 3b가
  이미 정해 두었다.
- 검사 15 — 음성. 듣는 프로세스가 없을 때 체인이 그것을 실패로 읽나.
  기다릴 필요가 없다 — 결정 10 덕분에 `read`가 즉시 EOF를 보고 빈 값으로
  돌아온다. 판정은 받은 것이 비어 있다는 것이고, `rc`는 절대 안 본다
  (실측 4: 음성과 양성이 `rc`에서 같은 값이다).
- 게이트 한 판. 열두 체인이 3/3인 것과 시간 증가분을 잰다.

M2가 들어가면서 조심할 것 둘. 하나, 검사 14를 검사 13 뒤에 놓으면 그 타이핑이
프롬프트를 다시 그리게 만들고 그때 fish의 job 종료 줄이 나타난다(실측 12).
`wait_for_screen`의 패턴이 그 줄과 안 부딪치는지 본다. 둘, 판정 글자가
`inm2-` 접두사여야 한다 — 같은 체인 안의 검사 넷이 서로의 글자를 안 밟게
하려는 것이고 근거가 design 결정 8이다.

M2의 끝 기준은 검사 넷이 통과하는 것, 반사실 둘이 각각 겨냥한 자리에서 죽는
것, 그리고 게이트가 3/3인 것이다.
...
`net/check.sh`는 단독으로 27.955초에 돈다(NW-M1에서 8.954초 · NW-M2에서
17.082초 · NW-M3에서 22.911초였다 — 디스크 굽기와 리스 대기, M3의 타이핑
78키, 그리고 IN-M1의 타이핑 87키가 차례로 늘었다). 단독 실행은 아래 "명령
모음"에 있다. 게이트가 이 체인을 세 번 도니 IN-M1의 예상 증가분이 약 15초이고,
그 크기는 게이트의 잡음(±3분) 안이라 다음 게이트에서 잡히지 않을 것이다.
