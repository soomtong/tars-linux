...
## 그 앞의 milestone — Userland Tools UT-M3 (2026-09-11)

**도구 65개와 링크 넷, 그리고 git**
...
으로 closure를 재라. **M1이 그것을 안 해서 라이브러리 둘을 놓쳤고, M2가 30초
...
**다시 재는 법**(잰 스크립트는 `/tmp`에 있었고 이 세션과 함께 사라진다).
루트 게이트 한 판(23분)으로는 이 종류가 안 잡힌다 — **체인을 세 번 돌리는
것으로도 모자라고, 같은 부팅을 여덟 번 이상 해야 보인다.**

1. 컨테이너 안에서 `gate_lib.sh`를 source하고, 체인과 **같은 QEMU 인자**로
   부팅해 monitor에 붙는 루프를 여덟 번 돈다(포트는 회차마다 다르게).
2. 체인이 치는 **순서 그대로** 친다. **`ps ax`가 앞에 있어야 재현된다** —
   `fd` 하나만 여덟 번 치면 8/8이다.
3. 판정은 옛 모양(`sleep 2` 뒤 한 번 `grep`)으로 하고, 실패한 회차의
   **마지막 화면 줄을 찍어 본다.** 거기에 찾던 글자가 있으면 도구가 아니라
   검사가 이른 것이다 — **이 한 줄이 이번 진단의 전부였다.**
