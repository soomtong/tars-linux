# HANDOFF: **Shell Memory가 닫혔다 — 다음 서브프로젝트를 고를 차례다**

## 지금 어디인가

`main`이 깨끗하다. **Shell Memory(SM-M0~M2)가 2026-09-12에 닫혔고**
`5c45cc4`까지 커밋됐다. 그 세션의 루트 게이트가 **28분 03.23초에 3/3 통과**했고
FAIL이 0이었다.
...
플래그 **끝**이 `q`인 것만 찾아서 `-aqE` 한 자리를 놓쳤다.
...
안에서 하면 **6/6 정상**이다. `--verbose`를 줘도 한 줄도 더 안 나오고
**컴파일 에러와 구분이 안 되는 모양**이라 더 나쁘다. 아래 "명령 모음"의 첫
형태로 친다.
...
HI-M3 · RM-M1 · UT-M3), 3/3. **가장 최근 값은 28분 03.23초다**(2026-09-12,
SM-M2). 기준선의 역사는 `project_gate_latency`에 있다 — 54분 15초에서 GL-M0~M3이
...
따로 세어** 확인한다 — 순수 추가면 **지운 줄이 0인 것**이 증명이고, 지우는
...
- ~~design doc 넷의 낡은 `Status:` 줄~~ — 2026-08-31에 고쳤다. **넷 다 계획한
  milestone을 전부 끝내 놓고 표시만 안 한 것**이었다. 그래서 지금은 끝낼 때
...
- `hangul.zig` — 한글 오토마타(HI-M0~M3). **시스템 콜도 `vt.zig`도 `drm.zig`도
  ...
  자리만 채운다**)과 자판 표들에 **`comptime` 앵커**가 박혀 있다 — 표 중간에
  줄을 끼우면 컴파일이 막힌다. `Syllable`은 인덱스를 담고 `jong`의 null이
  "받침 없음"이다(0을 안 쓴다). `codepoint()`는 **못 그리는 조합이면 null**이고
...
- `vt.zig` — `Screen`. `cells()`가 색·inverse·매치·선택·preedit·커서를 전부
  ...
  정한다). `copyApply`는 **모든 이동 수단이 통과하는 문**이고,
  `findCurrentIndex`가 **라이브러리 내부 필드 `selected.idx`를 읽는 유일한
  자리**다. `copyExit`은 뷰포트도 preedit도 안 되돌린다.
...
- `config_test.zig`의 **`expectQuietSeed`** — 씨앗 rc가 **부팅할 때 한 글자도
  안 찍는 것**을 호스트에서 막는다. 쓸 수 있는 줄은 주석 · `alias` · `command -v`
...
- `tools/check.sh` — 검사 열여섯(UT·SM). 바이너리 목록은
  **`kernel/guest_tools.sh` 한 파일**에 있고 `make_initrd.sh`와 이 체인이 같은
  배열을 본다.
- `machine/check.sh` — 실기 경로(RM). fish 인사말을 **UEFI 부팅의 마커**로 쓴다
