# HANDOFF: 문서를 정리했다 — 다음 서브프로젝트를 고를 차례다

## 지금 어디인가

`main`이 깨끗하다. Shell Memory(SM-M0~M2)가 2026-09-12에 닫혔고
`5c45cc4`까지 커밋됐다. 그 세션의 루트 게이트가 28분 03.23초에 3/3 통과했고
FAIL이 0이었다.
...
## 그 뒤에 문서 정리를 했다 (2026-09-12, 코드는 안 건드렸다)

커밋 둘이다. 셋째 커밋이 이 절과 새 기억을 더한다.
...
남겨야 하는 자리는 `docs/decisions/feedback_no_emphasis.md`에 있다.

정리 뒤에 sanity 검사를 일곱 했고 전부 통과했다 — 두 커밋이 건드린 non-md
...
남은 것 둘. 소스 주석에는 강조가 그대로 있고(`.zig`·`.sh`의 2,209줄), 강조를
지운 줄의 줄바꿈은 다시 잡지 않았다. 이유는 둘 다
`feedback_no_emphasis`에 적혀 있다.
...
- [ ] 소스 주석의 `**` 강조 2,209줄(`vt.zig` 229개 · `main.zig` 214개 ·
      `input.zig` 207개 · `hangul/check.sh` 150개 …). 2026-09-12에 문서만
      했다. Zig에는 `**`가 실제 연산자라 주석 줄만 골라내는 판별이 하나 더
      필요하다 — `feedback_no_emphasis`.
