# HANDOFF: Inbound Network(IN)를 열었고 M0이 끝났다

## 지금 어디인가

서브프로젝트 Inbound Network(IN)가 2026-09-14에 열렸고 같은 날 M0이 끝났다.
받는 방향이 실제로 선다는 것을 측정으로 확인했고, 아직 `net/check.sh`에는
한 줄도 안 넣었다. 다음 할 일은 IN-M1 plan을 쓰는 것이다.
...
루트 게이트는 열두 체인 3/3이고 30분 57.86초다. IN-M0은 저장소의 코드
파일을 한 글자도 안 바꿨으므로 게이트를 안 돌렸고 이 값이 그대로 유효하다.
...
- 포트는 45465·45466으로 정해져 있다(monitor 대역 45455~45464·45471과 안
  겹친다). `hostfwd` 둘을 한 `-netdev` 값에 쉼표로 이어 붙이는 것이
  이 QEMU에서 그대로 먹는다.
...
## 바로 다음에 할 것 — IN-M1 plan을 쓴다

IN-M0이 끝났고 손에 든 일이 IN-M1이다. design의 "Milestone 셋" 절에 M1의
윤곽이 있고, M0의 "M1에 넘기는 것" 일곱이 그 위에 구체적인 값을 얹어 두었다.

M1이 하는 일은 `net/check.sh`에 `hostfwd` 한 줄과 검사 둘을 더하는 것이다.
검사 12가 게스트가 포트를 열었는지를 `/proc/net/tcp`의 `1F90`으로 보고,
검사 13이 체인이 붙어 `inm1-inbound-ok`를 읽었는지를 본다.

plan을 쓰기 전에 정해야 하는 것이 하나 있다 — 읽기의 종료 조건(design 위험
7). 안 정하면 게이트에 10초가 붙는다. 후보가 둘이고 저울질할 것은 이렇다.

- 체인에서 `read -r -t <초> <변수> <&3`로 한 줄만 읽는다. 게스트에 요구하는
  것이 없는 대신 체인이 "판정 글자가 한 줄"이라는 형식을 가정하게 된다.
- 게스트의 `nc`에 `-q 0`을 준다. 형식 가정이 없는 대신 게이트가 치는 명령이
  길어지고 `nc.traditional`의 그 옵션이 실제로 도는지를 확인해야 한다.

M1의 끝 기준은 체인이 단독으로 통과하는 것과, `hostfwd` 줄만 뺀 반사실이
검사 13에서 죽는 것이다. 반사실이 검사 12에서 죽으면 그것도 값진 결과이고
(리스너와 포워딩이 서로 엮여 있다는 뜻이므로) 그 사실을 적는다.

그 뒤 IN-M2가 반대 방향(검사 14)과 음성(검사 15)을 넣고 게이트를 돌린다.
반대 방향의 모양은 M0 실측 3b가 이미 정해 두었다.
