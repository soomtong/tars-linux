# HANDOFF: Find Paste가 시작됐다 — FP-M0을 끝냈고 다음은 FP-M1

## 지금 어디인가

`main`, working tree 깨끗함. **Find Paste(FP)를 착수했고 FP-M0이 2026-09-09에
끝났다.** `vt.zig`에 `findPaste`가 서서 클립보드의 **첫 줄**을 검색어에
붙인다. **아직 부르는 자리가 `vt_test`뿐이라 사람이 검색창에 붙여넣을 수는
없다** — 의도된 상태이고 FP-M1이 배선한다.

**게이트는 안 돌렸다.** FP-M0은 부팅하는 바이너리의 동작을 한 글자도 안
바꾼다(더한 줄 164, 지운 줄 0). 마지막 게이트 값은 SH가 남긴 **18분 58.81초**다.

| | 파일 | 커밋 |
|---|---|---|
| design | `docs/superpowers/specs/2026-09-09-tars-find-paste-design.md` | `5d2f29e` |
| plan (FP-M0) | `docs/superpowers/plans/2026-09-09-tars-find-paste-fp-m0.md` | `7784c3c` |
| FP-M0 Task 1 | `vt.zig`의 `findPaste` · `vt_test` 검사 55~59 | `3a31702` |

```bash
git status --short     # 비어 있어야 한다
docker run --rm -v "$PWD":/workspace -w /workspace/terminal tars-devcontainer \
  bash -c 'zig build test'   # vt_test의 마지막이 검사 59, PASS
```

**`PASS`가 넷인 것이 정상이다.** 다섯 바이너리가 다 돌지만
`status_test.zig`만 `PASS`를 안 찍는다(IS-M1이 만들 때부터 그랬다).
**세는 것으로 판정하지 말 것** — 종료 코드와 `*_test:` 접두사 다섯을 본다.

## 이 세션도 편집을 Claude Code가 했다 — **다음 milestone은 다시 기본 규칙이다**

사용자가 2026-09-09에 "외출해야 하니 이 milestone의 구현을 대신 해 달라"고
정했다. `CLAUDE.md`의 기본 규칙("파일 편집은 사용자가")에 대한 **FP-M0 한정
예외**이고, SH 세션의 예외와 같은 종류이며 범위만 더 좁다.
**FP-M1부터는 사용자가 편집한다.**

## 바로 다음에 할 것 — **FP-M1**

design의 milestone 절이 Task 넷을 이미 자세히 적어 뒀다. **plan은 착수할 때
쓴다.**

| Task | 무엇 | 판정 |
|---|---|---|
| 1 | `Cmd+V`의 판단을 **1.35번 단계 한 자리로** 모으고 기존 두 줄을 지운다 | `input_test` 58·59 + **`copy/check.sh` 한 체인** |
| 2 | `main.zig`의 목적지 갈래와 `dumpFindPaste` | 게이트 |
| 3 | `hangul/check.sh` 검사 20 | 게이트 |
| 4 | 전체 게이트 아홉 체인 3/3 | — |

**Task 1과 2 사이에 의도된 중간 상태가 있다** — 프롬프트에서 `Cmd+V`가
클립보드를 **셸로** 쓴다. SH-M1이 끝난 시점에 프롬프트의 한글이 글리프 셋으로
깨져 보였던 것과 같은 종류이고, Task 2가 닫는다.

## FP-M0이 실행으로 증명한 것 — **다시 조사하지 말 것**

**1. 여러 줄을 yank하면 개행이 정말 들어간다. `0A` 한 바이트이고 CR이 없다.**
결정 5 전체가 이 전제 위에 서 있어서 임시 프로브로 쟀다(`vt_test`에 넣었다
되돌렸다).
...
**전제를 검사에 적기 전에 재 본 것이 이 milestone에서 가장 값진 30초였다.**
...
런타임이 아니라 컴파일 실패인 것이 정상이다. 부를 함수가 없었다.

**4. 대조군이 공짜로 나오는 자리가 있었다.** 검사 54가 끝난 자리의
화면(`um`)이 마침 `y`를 한 번도 안 눌러 클립보드가 비어 있다. 그것을 검사 55로
먼저 두지 않으면 **`findPaste`가 늘 무언가를 넣는 구현도 나머지 넷을 전부
통과한다.** design의 표는 원래 이 검사를 58번 자리에 뒀는데, plan을 쓰다
발견해서 앞으로 옮기고 design도 함께 고쳤다.

## FP가 세운 것 — 저장소에 서 있는 것 하나

1. **`vt.zig`의 `findPaste`** — 클립보드의 첫 줄을 needle에 붙이고 넣은
   바이트 수를 돌려준다. 개행에서 자르는 것이 본체이고, **넣는 일은
   `findBytes`에 그대로 넘긴다** — 통째로 받거나 거절하는 규칙도, 프롬프트가
   닫혀 있으면 아무 일도 안 하는 규칙도 그쪽 한 자리에만 적힌다.
2. **검사들** — `vt_test` 55(대조군: 빈 클립보드) · 56(한 줄) · 57(이어
   붙기) · 58(**여러 줄은 첫 줄만**) · 59(넘치면 통째로 거절).

## FP가 무엇인가

**`/` 프롬프트에서 `Cmd+V`가 클립보드를 검색어에 붙인다.** SH design의
비목표 절이 다음 후보로 지목하고 근거까지 모아 둔 것이다.
...
2. **`commitHangul()`을 명시적으로 부른다.** 끌어올린 자리가 `hangulLayer`보다
   앞이라서다. **새 통로가 안 는다** — `readKeys:1494`의 `takeCommit()`이
   action과 무관하게 돌면서 `to_needle`로 목적지를 가른다.
